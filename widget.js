(function () {
  ...
  const isDemo = currentScript ? (currentScript.getAttribute('data-demo') === 'true') : false;
  ...
  // 1. 홈페이지 최상단 데모 안내 배너 주입 (데모 시연일 때만)
  if (isDemo) {
  ...
  // 2. 홈페이지 우측 하단 플로팅 라이브 뱃지 위젯 주입 (항상 노출)
  ...
  // 3. 홈페이지 좌측 하단 도입 문의 플로팅 버튼 주입 (데모 시연일 때만)
  let leadBtn = null;
  let leadModal = null;
  if (isDemo) {
