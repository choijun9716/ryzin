...
// 1. replace alert with custom toast
const alertTarget = `      e.preventDefault();
      alert('상품 구매 페이지로 이동합니다: ' + p.name);`;
const alertNew = `      e.preventDefault();
      showToast('상품 구매 페이지로 이동합니다: ' + p.name);`;

content = content.replace(alertTarget, alertNew);

// 2. Add toast function and prevent pause
const endTarget = `});

// 하트 애니메이션 키프레임 (JS로 동적 추가)`;

const newEnd = `  // 모바일 키보드 열림 등으로 인해 비디오가 일시정지되는 현상 방지
...
content = content.replace(keyframeTarget, keyframeNew);
