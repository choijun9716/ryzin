...
설정을 저장한 후 라이브를 시작할 수 있습니다.`),document.getElementById(`cfg-stream`).focus(),document.getElementById(`cfg-stream`).style.borderColor=`#ef4444`,document.getElementById(`cfg-stream`).style.boxShadow=`0 0 0 3px rgba(239,68,68,0.15)`,setTimeout(()=>{document.getElementById(`cfg-stream`).style.borderColor=``,document.getElementById(`cfg-stream`).style.boxShadow=``},3e3);return}if(o.isLive=!o.isLive,o.isLive){o.liveStartTime=new Date().toISOString();let e=document.getElementById(`cfg-liveStartTime`);e&&(e.value=new Date(new Date().getTime()-new Date().getTimezoneOffset()*6e4).toISOString().slice(0,16))}e.target.textContent=o.isLive?`라이브 종료`:`라이브 시작`,e.target.className=`action-btn ${o.isLive?`btn-danger-solid`:`btn-success-solid`}`,e.target.style.cssText=`flex:1; justify-content:center; padding:14px; font-size:15px;`,u(),st(t,o,s,c,!0)});let h=async(e,t,n)=>{if(e){document.getElementById(t).style.opacity=`0.5`;try{let r=n===`logoUrl`,s=await i(await a(e,r?256:1080,r?256:1920,r?.88:.82));o[n]=s;let c=document.getElementById(t);if(c.src=s,c.style.display=`block`,n===`shareImageUrl`){let e=document.getElementById(`og-preview-img`),t=document.getElementById(`og-preview-img-placeholder`),n=document.getElementById(`og-img-placeholder`);e&&(e.src=s,e.style.display=`block`),t&&(t.style.display=`none`),n&&(n.style.display=`none`)}u()}catch(e){console.error(`이미지 업로드 오류:`,e),alert(`이미지 업로드 실패: `+e.message)}finally{document.getElementById(t).style.opacity=`1`}}},_=async e=>{if(!e)return;let t=document.getElementById(`like-preview`),n=document.getElementById(`like-preview-placeholder`),r=document.getElementById(`btn-clear-like-icon`);t.style.opacity=`0.5`;try{let s=``;s=e.size<1.2*1024*1024?await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result.split(`,`)[1]),n.readAsDataURL(e)}):await a(e,512,512,.9);let c=await i(s);o.likeImageUrl=c,t.src=c,t.style.display=`block`,n&&(n.style.display=`none`),r&&(r.style.display=`block`),u()}catch(e){console.error(`응원 이미지 업로드 오류:`,e),alert(`응원 이미지 업로드 실패: `+e.message)}finally{t.style.opacity=`1`}};document.getElementById(`cfg-logoFile`).addEventListener(`change`,e=>h(e.target.files[0],`logo-preview`,`logoUrl`)),document.getElementById(`cfg-thumbnailFile`).addEventListener(`change`,e=>h(e.target.files[0],`thumbnail-preview`,`thumbnailUrl`)),document.getElementById(`cfg-shareImageFile`).addEventListener(`change`,e=>h(e.target.files[0],`share-image-preview`,`shareImageUrl`)),document.getElementById(`cfg-likeFile`).addEventListener(`change`,e=>_(e.target.files[0])),document.getElementById(`btn-clear-like-icon`).addEventListener(`click`,()=>{o.likeImageUrl=``;let e=document.getElementById(`like-preview`),t=document.getElementById(`like-preview-placeholder`),n=document.getElementById(`btn-clear-like-icon`);e&&(e.src=``,e.style.display=`none`),t&&(t.style.display=`block`),n&&(n.style.display=`none`),u()}),Z&&Z.from(`live_control`).select(`*`).eq(`live_id`,t).maybeSingle().then(({data:e,error:t})=>{if(t)throw t;if(e){let t=parseInt(e.cum_viewers)||0,n=parseInt(e.viewers)||0,r=parseInt(e.hearts)||0,i=document.getElementById(`cfg-cumViewers`);i&&(i.value=t);let a=document.getElementById(`cfg-viewers`);a&&!a.matches(`:focus`)&&(a.value=n);let o=document.getElementById(`cfg-hearts`);o&&!o.matches(`:focus`)&&(o.value=r),s.cumViewers=t,s.viewers=n,s.hearts=r,d()}}).catch(e=>console.warn(`Failed to fetch stats from Supabase`,e))},D=()=>{x.innerHTML=`
    ...
    `;let e=document.getElementById(`admin-chat-input`),n=document.getElementById(`admin-chat-list`),r=document.getElementById(`admin-nickname-input`),i=document.getElementById(`admin-color-input`),a=document.getElementById(`admin-color-code`),s=document.getElementById(`admin-bg-color-input`),c=document.getElementById(`admin-bg-color-code`),u=async()=>{let e=document.getElementById(`winner-table-body`);if(e)try{if(!Z)return;let{data:n,error:r}=await Z.from(`live_winners`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!1});if(r)throw r;if(!n||n.length===0){e.innerHTML=`<tr><td colspan="5" style="text-align:center; padding:30px; color:#94a3b8;">당첨자 제출 목록이 없습니다.</td></tr>`;return}e.innerHTML=n.map(e=>{let t=new Date(e.created_at).toLocaleTimeString(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`});return`
          ...
          `}).join(``)}catch(e){console.warn(`Failed to fetch winners`,e)}};document.getElementById(`btn-refresh-winners`)?.addEventListener(`click`,u),u();let d=setInterval(u,1e4);r&&r.addEventListener(`input`,()=>{localStorage.setItem(`ryzin_admin_nickname`,r.value.trim())}),i&&i.addEventListener(`input`,()=>{localStorage.setItem(`ryzin_admin_color`,i.value),a&&(a.textContent=i.value)}),s&&s.addEventListener(`input`,()=>{localStorage.setItem(`ryzin_admin_bg_color`,s.value),c&&(c.textContent=s.value)});let f=!1,m=async()=>{let a=e.value.trim();if(!a||f)return;f=!0;let o=(r?r.value.trim():``)||`관리자`,c=i?i.value:`#ffca28`,l=s?s.value:`#e50914`,u=`${o}|${c}|${l}`,d=Date.now(),p=document.createElement(`div`);p.style.cssText=`margin-bottom:8px; padding:8px 12px; border-radius:10px; background:${l}22; border-left:4px solid ${c}; display:flex; flex-direction:column; gap:2px;`,p.innerHTML=`<span style="font-weight:700; color:${c}; font-size:12px;">${o}</span><span style="font-size:13px; color:#1e293b;">${a}</span>`,n.innerHTML.includes(`실시간 채팅`)&&(n.innerHTML=``),n.appendChild(p),n.scrollTop=n.scrollHeight,e.value=``;try{if(!Z)return;await Z.from(`live_chats`).insert([{live_id:t,created_at:d,nickname:u,content:a}])}catch(e){console.warn(`Admin chat send failed`,e)}finally{f=!1}};document.getElementById(`btn-send-chat`).addEventListener(`click`,m),e.addEventListener(`keypress`,e=>{e.key===`Enter`&&m()}),document.getElementById(`btn-clear-chats`).addEventListener(`click`,async()=>{if(confirm(`📡 이 라이브 방송의 모든 실시간 채팅 내역을 초기화(영구 삭제)하시겠습니까?
이 작업은 복구할 수 없습니다.`)){let e=document.getElementById(`btn-clear-chats`);e.disabled=!0,e.textContent=`초기화 중...`;try{if(!Z)throw Error(`Supabase client가 로드되지 않았습니다.`);let{error:e}=await Z.from(`live_chats`).delete().eq(`live_id`,t);if(e)throw e;alert(`채팅 내역이 성공적으로 초기화되었습니다!`),n.innerHTML=`<div style="color:#94a3b8; text-align:center; padding-top:70px; font-weight:500;">
            <div style="font-size:24px; margin-bottom:8px;">💭</div>
            실시간 채팅 내역이 여기에 표시됩니다.
          </div>`}catch(e){alert(`채팅 내역 초기화 실패: `+e.message)}finally{e.disabled=!1,e.textContent=`채팅 내역 초기화`}}});let h=document.getElementById(`bot-chat-list`),g=document.getElementById(`bot-interval`);h.addEventListener(`input`,()=>{l.list=h.value,p()}),g.addEventListener(`input`,()=>{l.interval=parseInt(g.value)||10,p()});let _=null,v=!1;document.getElementById(`btn-toggle-bot`).addEventListener(`click`,()=>{v=!v;let e=document.getElementById(`bot-icon`),r=document.getElementById(`bot-text`),i=document.getElementById(`btn-toggle-bot`);if(v){let a=h.value.split(`
`).map(e=>e.trim()).filter(e=>e.includes(`|`));if(a.length===0){alert(`닉네임|내용 형식으로 1줄 이상 입력해주세요.`),v=!1;return}e.textContent=`⏸`,r.textContent=`채팅 봇 중지`,i.className=`action-btn btn-danger-solid`,i.style.cssText=`width:100%; justify-content:center; padding:14px; font-size:15px; gap:8px;`;let o=parseInt(g.value)||10;_=setInterval(async()=>{let[e,...r]=a[Math.floor(Math.random()*a.length)].split(`|`),i=r.join(`|`).trim();if(!e||!i)return;let o=Date.now(),s=document.createElement(`div`);s.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`,s.innerHTML=`<span style="font-weight:700; color:#64748b;">${e.trim()}:</span> ${i}`,n.innerHTML.includes(`실시간 채팅`)&&(n.innerHTML=``),n.appendChild(s),n.scrollTop=n.scrollHeight;try{if(!Z)return;await Z.from(`live_chats`).insert([{live_id:t,created_at:o,nickname:e.trim(),content:i}])}catch(e){console.warn(`Bot chat failed`,e)}},o*1e3)}else _&&clearInterval(_),e.textContent=`▶`,r.textContent=`채팅 봇 시작`,i.className=`action-btn btn-primary-solid`,i.style.cssText=`width:100%; justify-content:center; padding:14px; font-size:15px; gap:8px;`});let y=0,b=null,S=(e,t,r=!1)=>{n.innerHTML.includes(`실시간 채팅 내역이 여기에`)&&(n.innerHTML=``);let i=document.createElement(`div`);i.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`+(r?`opacity:0.72;`:``),i.innerHTML=`<span style="font-weight:700; color:${e===`관리자`?`#3b82f6`:`#64748b`};">${e}:</span> ${t}`,n.appendChild(i),r||(n.scrollTop=n.scrollHeight)};(async()=>{if(Z)try{let{data:e,error:r}=await Z.from(`live_chats`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!0}).limit(100);if(r)throw r;e&&Array.isArray(e)&&(e.forEach(e=>{S(e.nickname||`?`,e.content||``,!0),y=parseInt(e.created_at)||0}),setTimeout(()=>{n.scrollTop=n.scrollHeight},100))}catch(e){console.warn(`Failed to load chat history`,e)}})(),Z&&(b=Z.channel(`admin-chat-channel-${t}`).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`live_chats`,filter:`live_id=eq.${t}`},e=>{let t=e.new;t&&parseInt(t.created_at)>y&&(S(t.nickname||`?`,t.content||``,!1),y=parseInt(t.created_at))}).subscribe()),x.addEventListener(`adminTabLeave`,()=>{b&&Z.removeChannel(b),_&&clearInterval(_),d&&clearInterval(d)});let C=document.getElementById(`btn-winner-start`),w=document.getElementById(`btn-winner-cancel`),T=o.winner_name&&o.winner_name.startsWith(`구매인증`)?`구매인증`:`소통왕`,E=document.querySelectorAll(`.type-segment-btn`);E.forEach(e=>{e.addEventListener(`click`,()=>{E.forEach(e=>{e.classList.remove(`active`),e.style.background=`transparent`,e.style.color=`#64748b`}),e.classList.add(`active`),e.style.background=`#fab005`,e.style.color=`#fff`,T=e.dataset.type})}),C&&C.addEventListener(`click`,async()=>{let e=document.getElementById(`winner-announce-text`).value.trim(),n=parseInt(document.getElementById(`winner-announce-min`).value)||1;if(!e){alert(`당첨자 닉네임을 입력해 주세요.`);return}C.disabled=!0,C.textContent=`적용 중...`;let r=`${T}|${e}`,i=Date.now()+n*60*1e3;try{if(!Z)return;let{error:e}=await Z.from(`live_control`).update({winner_name:r,winner_timestamp:i,updated_at:new Date().toISOString()}).eq(`live_id`,t);if(e)throw e;o.winner_name=r,o.winner_timestamp=i,alert(`🎉 ${T} 당첨 배너 노출이 시작되었습니다!`),D()}catch{alert(`시작 처리에 실패했습니다.`)}finally{C.disabled=!1,C.textContent=`시작`}}),w&&w.addEventListener(`click`,async()=>{w.disabled=!0,w.textContent=`종료 중...`;try{if(!Z)return;let{error:e}=await Z.from(`live_control`).update({winner_timestamp:0,updated_at:new Date().toISOString()}).eq(`live_id`,t);if(e)throw e;o.winner_timestamp=0,alert(`당첨 배너 노출이 종료되었습니다.`),D()}catch{alert(`종료 처리에 실패했습니다.`)}finally{w.disabled=!1,w.textContent=`종료`}})},O=()=>c.map((e,t)=>{let n=e.clicks||0;return`
