...
`).map(e=>e.trim()).filter(e=>e.includes(`|`)),p.length===0){alert(`닉네임|내용 형식으로 1줄 이상 입력해주세요.`),d=!1;return}f>=p.length&&(f=0),A();let e=parseInt(k.value)||10;u&&clearInterval(u),u=setInterval(async()=>{if(f>=p.length){clearInterval(u),u=null,d=!1,A();return}let[e,...n]=p[f++].split(`|`),r=n.join(`|`).trim();if(!e||!r)return;let i=Date.now(),a=document.createElement(`div`);a.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`,a.innerHTML=`<span style="font-weight:700; color:#64748b;">${e.trim()}:</span> ${r}`,g.innerHTML.includes(`실시간 채팅`)&&(g.innerHTML=``),g.appendChild(a),g.scrollTop=g.scrollHeight;try{if(!X)return;await X.from(`live_chats`).insert([{live_id:t,created_at:i,nickname:e.trim(),content:r}])}catch(e){console.warn(`Bot chat failed`,e)}},e*1e3)}else u&&clearInterval(u),u=null,A()});let j=document.getElementById(`auto-reply-active`),M=document.getElementById(`btn-add-ar`),N=document.getElementById(`ar-title`),P=document.getElementById(`ar-keywords`),F=document.getElementById(`ar-answer`),I=document.getElementById(`ar-list-container`),L=()=>{if(I){if(!l.autoReplyRules||l.autoReplyRules.length===0){I.innerHTML=`<div style="text-align:center; padding:20px; color:#94a3b8; font-size:13px; font-weight:500;">등록된 자동응답 규칙이 없습니다.</div>`;return}I.innerHTML=l.autoReplyRules.map((e,t)=>`
      ...
      `).join(``),I.querySelectorAll(`.btn-del-ar`).forEach(e=>{e.addEventListener(`click`,e=>{let t=parseInt(e.target.dataset.index);l.autoReplyRules.splice(t,1),_(),L()})})}};j&&j.addEventListener(`change`,e=>{l.autoReplyActive=e.target.checked,_()}),M&&M.addEventListener(`click`,()=>{let e=N.value.trim(),t=P.value.trim(),n=F.value.trim();if(!t||!n){alert(`키워드와 답변 내용을 모두 입력해주세요.`);return}l.autoReplyRules||=[],l.autoReplyRules.push({title:e,keywords:t,answer:n}),_(),N.value=``,P.value=``,F.value=``,L()}),L();let te=0,ne=null,re=(e,n,r=!1)=>{g.innerHTML.includes(`실시간 채팅 내역이 여기에`)&&(g.innerHTML=``);let i=document.createElement(`div`);i.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9; display:flex; justify-content:space-between; align-items:center;`+(r?`opacity:0.72;`:``);let a=e===`관리자`||e.includes(`|`)?`#3b82f6`:`#64748b`,l=e,u=!1;e.includes(`|`)?(l=e.split(`|`)[0],u=!0):e===`관리자`&&(u=!0);let d=!u&&l!==`?`?`<button class="btn-ban-user" data-nickname="${l}" style="background:#ef4444; color:#fff; border:none; border-radius:4px; padding:2px 8px; font-size:11px; font-weight:700; cursor:pointer; margin-left:8px; line-height:1.4; flex-shrink:0;">차단</button>`:``;i.innerHTML=`
        <div style="flex:1; min-width:0; word-break:break-all; font-size:13px;">
          <span style="font-weight:700; color:${a};">${l}:</span> ${n}
        </div>
        ${d}
      `;let f=i.querySelector(`.btn-ban-user`);f&&f.addEventListener(`click`,e=>{e.stopPropagation();let n=f.dataset.nickname;if(confirm(`📡 [${n}] 시청자를 차단하시겠습니까?\n차단 이후에는 이 시청자의 채팅 전송이 제한됩니다.`)){let e=o.bannedUsers?o.bannedUsers.split(`,`).map(e=>e.trim()).filter(e=>e):[];e.includes(n)||(e.push(n),o.bannedUsers=e.join(`,`),m(),jt(t,o,s,c,!0),alert(`[${n}] 님이 정상 차단되었습니다.`),ee())}}),g.appendChild(i),r||(g.scrollTop=g.scrollHeight)},ie=async()=>{if(X)try{let{data:e,error:n}=await X.from(`live_chats`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!1}).limit(300);if(n)throw n;e&&Array.isArray(e)&&(e.reverse().forEach(e=>{re(e.nickname||`?`,e.content||``,!0),te=parseInt(e.created_at)||0}),setTimeout(()=>{g.scrollTop=g.scrollHeight},100))}catch(e){console.warn(`Failed to load chat history`,e)}};window.updateAdminViewersDisplay=()=>{let e=document.getElementById(`cfg-viewers-display`);e&&(e.innerHTML=`${(s.viewers+(s.cumViewers||0)).toLocaleString()}명 <span style="font-size:11px; font-weight:normal; color:#64748b; margin-left:4px;">(방송+수동: ${s.viewers.toLocaleString()}, 누적: ${(s.cumViewers||0).toLocaleString()})</span>`);let t=document.getElementById(`cfg-cumViewers`);t&&(t.value=s.cumViewers||0);let n=document.getElementById(`cfg-hearts`);n&&(n.value=s.hearts||0)};let ae=null;ie(),X&&(ne=X.channel(`admin-chat-ui-channel-${t}`).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`live_chats`,filter:`live_id=eq.${t}`},e=>{let t=e.new;t&&parseInt(t.created_at)>te&&(re(t.nickname||`?`,t.content||``,!1),te=parseInt(t.created_at))}).subscribe()),X&&(ae=setInterval(async()=>{try{let{data:e,error:n}=await X.from(`live_control`).select(`viewers, cum_viewers, hearts`).eq(`live_id`,t).maybeSingle();e&&!n&&(s.viewers=parseInt(e.viewers)||0,s.cumViewers=parseInt(e.cum_viewers)||0,s.hearts=parseInt(e.hearts)||0,typeof window.updateAdminViewersDisplay==`function`&&window.updateAdminViewersDisplay())}catch{}},3e3)),T.addEventListener(`adminTabLeave`,()=>{ne&&X.removeChannel(ne),ae&&clearInterval(ae),w&&clearInterval(w)});let R=document.getElementById(`btn-winner-start`),z=document.getElementById(`btn-winner-cancel`),oe=o.winner_name&&o.winner_name.startsWith(`구매인증`)?`구매인증`:`소통왕`,se=T.querySelectorAll(`.type-segment-btn`);se.forEach(e=>{e.addEventListener(`click`,()=>{se.forEach(e=>{e.classList.remove(`active`),e.style.background=`transparent`,e.style.color=`#64748b`}),e.classList.add(`active`),e.style.background=`#3b82f6`,e.style.color=`#fff`,oe=e.dataset.type})}),R&&R.addEventListener(`click`,async()=>{let e=document.getElementById(`winner-announce-text`).value.trim(),n=parseInt(document.getElementById(`winner-announce-min`).value)||1;if(!e){alert(`당첨자 닉네임을 입력해 주세요.`);return}R.disabled=!0,R.textContent=`적용 중...`;let r=`${oe}|${e}`,i=Date.now()+n*60*1e3;try{if(!X)return;let{error:e}=await X.from(`live_control`).update({winner_name:r,winner_timestamp:i,updated_at:new Date().toISOString()}).eq(`live_id`,t);if(e)throw e;o.winner_name=r,o.winner_timestamp=i,ee()}catch{alert(`시작 처리에 실패했습니다.`)}finally{R.disabled=!1,R.textContent=`시작`}}),z&&z.addEventListener(`click`,async()=>{z.disabled=!0,z.textContent=`종료 중...`;try{if(!X)return;let{error:e}=await X.from(`live_control`).update({winner_timestamp:0,updated_at:new Date().toISOString()}).eq(`live_id`,t);if(e)throw e;o.winner_timestamp=0,ee()}catch{alert(`종료 처리에 실패했습니다.`)}finally{z.disabled=!1,z.textContent=`종료`}})},F=()=>c.map((e,t)=>{let n=e.clicks||0;return`
    ...
    `,X&&X.from(`live_control`).select(`products`).eq(`live_id`,t).maybeSingle().then(({data:t,error:n})=>{if(n)throw n;if(t&&t.products)try{let n=typeof t.products==`string`?JSON.parse(t.products):t.products;if(Array.isArray(n)||(n=[]),Array.isArray(n)){!c||c.length===0?c=n:c.forEach(e=>{let t=n.find(t=>t.name===e.name);t&&(e.clicks=parseInt(t.clicks)||0,!e.image&&t.image&&(e.image=t.image))}),g();let t=document.getElementById(`product-list-container`);t&&(t.innerHTML=F(),e())}}catch{}}).catch(e=>console.warn(`Failed to load product clicks from Supabase`,e));let e=()=>{let n=document.getElementById(`product-list-container`);n.querySelectorAll(`.price-input`).forEach(e=>{e.addEventListener(`input`,e=>{let t=e.target.value.replace(/[^0-9]/g,``);e.target.value=t?Number(t).toLocaleString():``})}),n.querySelectorAll(`input[data-field]`).forEach(t=>{t.addEventListener(`change`,t=>{let r=parseInt(t.target.dataset.idx),i=t.target.dataset.field;if(t.target.type===`checkbox`){if(c[r][i]=t.target.checked,i===`isLeadForm`){t.target.checked?c[r].url=`__LEAD_FORM__`:c[r].url===`__LEAD_FORM__`&&(c[r].url=``),n.innerHTML=F(),e();return}}else i===`price`||i===`normalPrice`?c[r][i]=t.target.value.replace(/[^0-9]/g,``):c[r][i]=t.target.value;if(i===`price`||i===`normalPrice`){let e=Number((c[r].normalPrice||``).toString().replace(/[^0-9]/g,``)),t=Number((c[r].price||``).toString().replace(/[^0-9]/g,``));if(e>0&&e>=t){c[r].discountRate=Math.floor((e-t)/e*100);let i=n.querySelector(`input[data-idx="${r}"][data-field="discountRate"]`);i&&(i.value=c[r].discountRate)}else{c[r].discountRate=0;let e=n.querySelector(`input[data-idx="${r}"][data-field="discountRate"]`);e&&(e.value=0)}}g()})}),n.querySelectorAll(`.prod-img-upload`).forEach(t=>{t.addEventListener(`change`,async t=>{let r=t.target.files[0];if(!r)return;let a=parseInt(t.target.dataset.idx),o=document.getElementById(`img-prev-${a}`);o&&(o.style.opacity=`0.5`);try{let t=await i(await new Promise((e,t)=>{let n=new FileReader;n.onload=()=>e(n.result.split(`,`)[1]),n.onerror=t,n.readAsDataURL(r)}));if(!t)throw Error(`서버로부터 다운로드 URL을 받지 못했습니다.`);c[a].image=t,o&&(o.src=t),g(),n.innerHTML=F(),e(),alert(`🎉 상품 이미지 변경 성공!`)}catch(e){console.error(e),alert(`❌ 상품 이미지 업로드 에러:
`+e.message)}finally{o&&(o.style.opacity=`1`)}})}),n.querySelectorAll(`.btn-deal-start`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx),a=parseInt(document.getElementById(`deal-min-${i}`).value);a>0&&(c[i].dealText=document.getElementById(`deal-text-${i}`).value||`깜짝딜 종료까지`,c[i].dealEndTime=Date.now()+a*60*1e3,g(),n.innerHTML=F(),e(),jt(t,o,s,c,!0),setTimeout(()=>alert(`${a}분 깜짝딜이 시작되었습니다!`),10))})}),n.querySelectorAll(`.btn-deal-cancel`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx);c[i].dealEndTime=0,g(),n.innerHTML=F(),e(),jt(t,o,s,c,!0)})}),n.querySelectorAll(`.btn-del-product`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx),i=c[r]?.name||`이 상품`;confirm(`정말 "${i}" 상품을 삭제하시겠습니까?`)&&(c.splice(r,1),g(),n.innerHTML=F(),e())})}),n.querySelectorAll(`.btn-move-up`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx);if(r>0){let t=c[r-1];c[r-1]=c[r],c[r]=t,g(),n.innerHTML=F(),e()}})}),n.querySelectorAll(`.btn-move-down`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx);if(r<c.length-1){let t=c[r+1];c[r+1]=c[r],c[r]=t,g(),n.innerHTML=F(),e()}})})};e(),document.getElementById(`btn-add-product`).addEventListener(`click`,()=>{c.push({id:Date.now(),name:`새 상품`,price:``,normalPrice:``,discountRate:0,image:`https://via.placeholder.com/72`,url:`#`}),g(),document.getElementById(`product-list-container`).innerHTML=F(),e()}),document.getElementById(`btn-save-products`).addEventListener(`click`,()=>{jt(t,o,s,c,!0),alert(`상품 목록이 적용되었습니다!`)})},L=()=>{T.innerHTML=`
      <div class="section-card">
        <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom:16px; border-bottom:1.5px solid #f1f5f9; margin-bottom:20px;">
...
`;e.forEach(e=>{let t=new Date(e.created_at).toLocaleString(`ko-KR`).replace(/,/g,``),r=(e.name||``).replace(/,/g,` `),i=(e.phone||``).replace(/,/g,` `);n+=`${t},${r},${i}\n`});let r=new Blob([`﻿`+n],{type:`text/csv;charset=utf-8;`}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=`상담DB_${t}.csv`,document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(i)})},te=v.querySelector(`#btn-back`);te&&te.addEventListener(`click`,()=>{T.dispatchEvent(new Event(`adminTabLeave`)),w(),u&&clearInterval(u),ie&&X.removeChannel(ie),re&&X.removeChannel(re),n(null)});let ne=v.querySelector(`#btn-refresh-preview`);ne&&ne.addEventListener(`click`,()=>{let e=v.querySelector(`#live-preview-iframe`);e&&(e.src=D)});let re=null;X&&(re=X.channel(`bot-sync-${t}`).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`live_chats`,filter:`live_id=eq.${t}`},e=>{let n=e.new;if(n&&l.autoReplyActive&&l.autoReplyRules&&l.autoReplyRules.length>0){let e=n.nickname||``;if(!e.includes(`|`)&&e!==`관리자`&&e!==`자동응답봇`){let e=(n.content||``).toLowerCase();for(let n of l.autoReplyRules)if(n.keywords.split(`,`).map(e=>e.trim().toLowerCase()).filter(e=>e).some(t=>e.includes(t))){setTimeout(async()=>{try{if(!X)return;await X.from(`live_chats`).insert([{live_id:t,nickname:`자동응답봇`,content:n.answer,created_at:Date.now().toString()}])}catch(e){console.warn(`Auto-reply failed`,e)}},600);break}}}}).subscribe());let ie=null;X&&(ie=X.channel(`admin-sync-${t}`).on(`postgres_changes`,{event:`UPDATE`,schema:`public`,table:`live_control`,filter:`live_id=eq.${t}`},e=>{let n=e.new;if(!n)return;o.brandName=n.title||``,o.title=n.subtitle||``;let r=n.profile_image||``;o.showSplash=!r.includes(`#nosplash`),o.logoUrl=r.replace(`#nosplash`,``),o.streamUrl=n.stream_url||``,o.showViewers=n.show_viewers!==!1,o.thumbnailUrl=n.thumbnail_url||``,o.liveStartTime=n.start_time||``,o.isLive=n.status===`ON`,o.shareTitle=n.share_title||``,o.shareDesc=n.share_desc||``,o.shareImageUrl=n.share_image||``,o.likeImageUrl=n.like_image_url||``,o.bannedWords=n.banned_words||``,o.bannedUsers=n.banned_users||``,n.winner_name!==void 0&&(o.winner_name=n.winner_name),n.winner_timestamp!==void 0&&(o.winner_timestamp=n.winner_timestamp),St(t,o);let i=(e,t)=>{let n=v.querySelector(`#`+e)||document.getElementById(e);n&&document.activeElement!==n&&(n.type===`checkbox`?n.checked=!!t:n.value=t)};i(`cfg-brandName`,o.brandName),i(`cfg-title`,o.title),i(`cfg-stream`,o.streamUrl),i(`cfg-showViewers`,o.showViewers),i(`cfg-liveStartTime`,o.liveStartTime),i(`cfg-shareTitle`,o.shareTitle),i(`cfg-shareDesc`,o.shareDesc),i(`cfg-bannedWords`,o.bannedWords),i(`cfg-bannedUsers`,o.bannedUsers);let a=v.querySelector(`#logo-preview`)||document.getElementById(`logo-preview`);a&&(a.src=o.logoUrl);let s=v.querySelector(`#thumbnail-preview`)||document.getElementById(`thumbnail-preview`);s&&(s.src=o.thumbnailUrl);let l=v.querySelector(`#like-preview`)||document.getElementById(`like-preview`);l&&(l.src=o.likeImageUrl,l.style.display=o.likeImageUrl?`block`:`none`);let u=v.querySelector(`#like-preview-placeholder`)||document.getElementById(`like-preview-placeholder`);u&&(u.style.display=o.likeImageUrl?`none`:`block`);let d=v.querySelector(`#btn-clear-like-icon`)||document.getElementById(`btn-clear-like-icon`);d&&(d.style.display=o.likeImageUrl?`block`:`none`);let f=v.querySelector(`#btn-toggle-live`)||document.getElementById(`btn-toggle-live`);if(f&&document.activeElement!==f&&(f.textContent=o.isLive?`라이브 종료`:`라이브 시작`,f.className=`action-btn ${o.isLive?`btn-danger-solid`:`btn-success-solid`}`),n.products&&Array.isArray(n.products)){let e=JSON.stringify(n.products);if(JSON.stringify(c)!==e){Array.isArray(c)||(c=[]),c.length=0,c.push(...n.products),Et(t,c);let e=v.querySelector(`#product-list-container`)||document.getElementById(`product-list-container`);e&&!e.contains(document.activeElement)&&typeof F==`function`&&(e.innerHTML=F())}}}).subscribe());let ae=b.querySelectorAll(`.tab-btn`),R=e=>{T.dispatchEvent(new Event(`adminTabLeave`)),ae.forEach(t=>t.classList.toggle(`active`,t.dataset.tab===e)),e===`config`?P():e===`chat`?ee():e===`product`?I():e===`leads`&&L()};ae.forEach(e=>{e.addEventListener(`click`,()=>R(e.dataset.tab))}),P()}function It(){let e=document.createElement(`div`),t=``;function n(){let r=U.getAll(`hosts`);if(t){let e=t.toLowerCase();r=r.filter(t=>t.name.toLowerCase().includes(e)||t.phone&&t.phone.includes(e))}let i=r.map(e=>{let t=U.getHostStats(e.id);return{...e,stats:t}});e.innerHTML=`
  ...
  `,i=document.createElement(`div`);if(i.style.cssText=`display: flex; gap: var(--space-3); justify-content: flex-end; width: 100%;`,t){let t=document.createElement(`button`);t.className=`btn btn-danger`,t.textContent=`삭제`,t.style.marginRight=`auto`,t.addEventListener(`click`,()=>{q(),st({title:`브랜드 삭제`,message:`"${n.name}" 브랜드를 삭제하시겠습니까?`,confirmText:`삭제`,danger:!0,onConfirm:()=>{U.delete(`brands`,e),J(`브랜드가 삭제되었습니다.`)}})}),i.appendChild(t)}let a=document.createElement(`button`);a.className=`btn btn-secondary`,a.textContent=`취소`,a.addEventListener(`click`,q);let o=document.createElement(`button`);o.className=`btn btn-primary`,o.textContent=t?`수정`:`등록`,o.addEventListener(`click`,()=>{let n=document.getElementById(`brand-name`).value.trim();if(!n){Y(`브랜드명을 입력해주세요.`);return}let r={name:n,companyName:document.getElementById(`brand-company`).value.trim(),category:document.getElementById(`brand-category`).value,manager:document.getElementById(`brand-manager`).value.trim(),phone:document.getElementById(`brand-phone`).value.trim(),email:document.getElementById(`brand-email`).value.trim(),businessNo:document.getElementById(`brand-biz`).value.trim(),taxInvoice:document.getElementById(`brand-tax`).value===`true`,address:document.getElementById(`brand-address`).value.trim(),memo:document.getElementById(`brand-memo`).value.trim()};t?(U.update(`brands`,e,r),J(`브랜드 정보가 수정되었습니다.`)):(r.id=R(`brand`),r.createdAt=new Date().toISOString().split(`T`)[0],U.create(`brands`,r),J(`브랜드가 등록되었습니다.`)),q()}),i.appendChild(a),i.appendChild(o),K({title:t?`브랜드 수정`:`브랜드 등록`,size:`lg`,content:r,footer:i})}function Ut(e){let t=document.createElement(`div`),n=U.getById(`brands`,e.id);if(!n)return t.innerHTML=`<div class="page-header"><div class="page-header-left"><h1 class="page-title">브랜드를 찾을 수 없습니다</h1></div></div>
  ...
  `,setTimeout(()=>{n.querySelector(`#btn-edit-info`)?.addEventListener(`click`,()=>{Yt(e)}),n.querySelectorAll(`.status-change-btn`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-status`);U.update(`projects`,e.id,{broadcastStatus:n}),J(`방송 상태가 "${z(n)}"(으)로 변경되었습니다.`),M.navigate(`/projects/${e.id}`)})}),n.querySelectorAll(`.settle-status-change-btn`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-status`);U.update(`projects`,e.id,{settleStatus:n}),J(`정산 상태가 "${oe(n)}"(으)로 변경되었습니다.`),M.navigate(`/projects/${e.id}`)})}),n.querySelector(`.tax-invoice-btn`)?.addEventListener(`click`,()=>{if(!t){Y(`등록된 브랜드 정보가 없어 세금계산서 상태를 변경할 수 없습니다.`);return}let n=!t.taxInvoice;U.update(`brands`,t.id,{taxInvoice:n}),J(`세금계산서 상태가 "${n?`발행완료`:`미발행`}"(으)로 변경되었습니다.`),M.navigate(`/projects/${e.id}`)})},0),n}function Yt(e){let t=U.getAll(`brands`),n=`
    <div class="form-grid">
      <div class="input-group">
...
`;return typeof a==`function`&&a(null,p),p}}));o((e=>{var t=xi(),n=Wi(),r=Ki(),i=qi();function a(e,r,i,a,o){let s=[].slice.call(arguments,1),c=s.length,l=typeof s[c-1]==`function`;if(!l&&!t())throw Error(`Callback required as last argument`);if(l){if(c<2)throw Error(`Too few arguments provided`);c===2?(o=i,i=r,r=a=void 0):c===3&&(r.getContext&&o===void 0?(o=a,a=void 0):(o=a,a=i,i=r,r=void 0))}else{if(c<1)throw Error(`Too few arguments provided`);return c===1?(i=r,r=a=void 0):c===2&&!r.getContext&&(a=i,i=r,r=void 0),new Promise(function(t,o){try{t(e(n.create(i,a),r,a))}catch(e){o(e)}})}try{let t=n.create(i,a);o(null,e(t,r,a))}catch(e){o(e)}}e.create=n.create,e.toCanvas=a.bind(null,r.render),e.toDataURL=a.bind(null,r.renderToDataURL),e.toString=a.bind(null,function(e,t,n){return i.render(e,n)})}))();function Ji(){U.isDemoMode&&(localStorage.setItem(`ryzin_is_demo_mode`,`false`),U.isDemoMode=!1,U.STORAGE_KEY=`livecommerce_erp_data`,U._load());let e=document.createElement(`div`);return e.className=`login-container`,e.innerHTML=`
  ...
  `,!await U.init()){e.innerHTML=`
    ...
    `;return}let t=()=>{if(e.querySelector(`.sidebar`))return;e.innerHTML=``,e.className=`app-layout`,e.appendChild(Qe());let t=document.createElement(`div`);t.className=`mobile-overlay`,t.onclick=()=>document.querySelector(`.sidebar`).classList.remove(`open`),e.appendChild(t);let n=document.createElement(`button`);n.className=`mobile-menu-btn`,n.innerHTML=`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg>`,n.onclick=()=>document.querySelector(`.sidebar`).classList.toggle(`open`),e.appendChild(n);let r=document.createElement(`main`);r.className=`main-content`,r.id=`page-content`,e.appendChild(r),M.setContainer(r)};M.beforeEach(n=>{let r=!!U.getCurrentUser();if(!r&&n!==`/login`)return`/login`;if(r&&n===`/login`)return`/`;if(r&&n===`/live_stream`){let e=U.getCurrentUser();if(U.isDemoMode||e&&(e.id===`demo`||e.role===`demo`))return`/`}return n===`/login`?(e.innerHTML=``,e.className=``,M.setContainer(e)):t(),!0}),M.register(`/login`,()=>Ji()),M.register(`/`,()=>pt()),M.register(`/live_stream`,()=>Nt()),M.register(`/projects`,()=>Gt()),M.register(`/projects/new`,()=>Gt()),M.register(`/projects/:id`,e=>qt(e)),M.register(`/hosts`,()=>It()),M.register(`/hosts/:id`,e=>Rt(e)),M.register(`/brands`,()=>Vt()),M.register(`/brands/:id`,e=>Ut(e)),M.register(`/finance`,()=>rn()),M.register(`/settlement`,()=>an()),M.register(`/contracts`,()=>cn()),M.register(`/marketing`,()=>fn()),M.register(`/crm`,()=>pn()),M.register(`/shop_manage`,()=>_a()),M.register(`/settings`,()=>mn()),M.start(),document.addEventListener(`click`,e=>{let t=e.target.closest(`a[href]`);t&&t.getAttribute(`href`).startsWith(`/`)&&!t.getAttribute(`target`)&&(e.preventDefault(),M.navigate(t.getAttribute(`href`)))})}Oa();