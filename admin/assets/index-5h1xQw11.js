  ...
  `,q({title:`프로젝트 상태 변경`,size:`md`,content:i,footer:o}),document.querySelectorAll(`.status-option`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.status-option`).forEach(e=>{e.className=`btn btn-secondary btn-sm status-option`,e.style.justifyContent=`flex-start`,e.style.fontSize=`12px`}),e.className=`btn btn-primary btn-sm status-option`,e.style.justifyContent=`flex-start`,e.style.fontSize=`12px`,a=e.getAttribute(`data-status`)})}),document.getElementById(`modal-cancel`)?.addEventListener(`click`,J),document.getElementById(`modal-view-detail`)?.addEventListener(`click`,()=>{J(),u.navigate(`/projects/${e}`)}),document.getElementById(`modal-save`)?.addEventListener(`click`,()=>{V.update(`projects`,e,{broadcastStatus:a}),J(),Y(`방송 상태가 "${C(a)}"(으)로 변경되었습니다.`);let t=document.getElementById(`page-content`);t&&(t.innerHTML=``,t.appendChild(He()))})}function Je(){let e=document.createElement(`div`);e.className=`dashboard-container`,e.style.display=`flex`,e.style.gap=`24px`,e.style.padding=`24px`,e.style.height=`calc(100vh - 48px)`,e.style.overflow=`hidden`,localStorage.getItem(`ryzin_live_config`)||localStorage.setItem(`ryzin_live_config`,JSON.stringify({brandName:`Ryzin Corp`,title:`단독 특가 라이브 방송 중!`,streamUrl:`https://ib3fjwlmgu0bwksrq8ao15010.edge.naverncp.com/live/video/ls-20260701130603-WkL1g/1080p-16-9/playlist.m3u8`,logoUrl:`https://ui-avatars.com/api/?name=R&background=0D8ABC&color=fff`,botEnabled:!0,showViewers:!0})),localStorage.getItem(`ryzin_live_stats`)||localStorage.setItem(`ryzin_live_stats`,JSON.stringify({viewers:1204,hearts:12040})),localStorage.getItem(`ryzin_live_products`)||localStorage.setItem(`ryzin_live_products`,JSON.stringify([{id:1,name:`[특가] 트루쿡 인덕션 프라이팬 3종 세트`,price:`49,900원`,image:`https://images.unsplash.com/photo-1584990347449-a6e81cb8860a?auto=format&fit=crop&q=80&w=200&h=200`,url:`#`},{id:2,name:`네티컬 딥 클렌징 앰플 기획세트`,price:`24,000원`,image:`https://images.unsplash.com/photo-1620916566398-39f1143ab7be?auto=format&fit=crop&q=80&w=200&h=200`,url:`#`},{id:3,name:`탐루미 수분폭탄 마스크팩 10매`,price:`12,900원`,image:`https://images.unsplash.com/photo-1596462502278-27bfdc403348?auto=format&fit=crop&q=80&w=200&h=200`,url:`#`}])),localStorage.getItem(`ryzin_live_chats`)||localStorage.setItem(`ryzin_live_chats`,JSON.stringify([]));let t=JSON.parse(localStorage.getItem(`ryzin_live_config`)),n=JSON.parse(localStorage.getItem(`ryzin_live_stats`)),r=JSON.parse(localStorage.getItem(`ryzin_live_products`)),i=`https://sheetdb.io/api/v1/3k5vdph36v8ej`,a=null,o=()=>{a&&clearTimeout(a),a=setTimeout(()=>{let e={업데이트시간:new Date().toISOString(),제목:t.brandName,부제목:t.title,프로필이미지:t.logoUrl,URL:t.streamUrl,시청자수:n.viewers,하트수:n.hearts,상품수:r.length,첫상품명:r.length>0?r[0].name:``,상품목록:JSON.stringify(r),시청자수노출:t.showViewers?`O`:`X`,썸네일URL:t.thumbnailUrl||``,시작일시:t.liveStartTime||``,방송상태:t.isLive?`ON`:`OFF`};fetch(`${i}?sheet=%EB%9D%BC%EC%9D%B4%EB%B8%8C%EA%B4%80%EC%A0%9C`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({data:[e]})}).catch(e=>console.warn(`SheetDB 연동 실패`,e))},1e3)},s=()=>{let e=document.getElementById(`live-preview-iframe`);e&&e.contentWindow&&e.contentWindow.postMessage({type:`sync_preview`,config:t,stats:n,products:r},`*`)},c=()=>{localStorage.setItem(`ryzin_live_config`,JSON.stringify(t)),window.dispatchEvent(new Event(`storage`)),s(),o()},l=()=>{localStorage.setItem(`ryzin_live_stats`,JSON.stringify(n)),s(),o()},u=()=>{localStorage.setItem(`ryzin_live_products`,JSON.stringify(r)),s()},d=document.createElement(`div`);d.style.flex=`1`,d.style.display=`flex`,d.style.flexDirection=`column`,d.style.gap=`24px`,d.style.overflowY=`auto`,d.style.paddingRight=`12px`;let f=document.createElement(`div`);f.className=`card`,f.innerHTML=`
    ...
    <div style="display:flex; gap:12px; margin-bottom:12px;">
      <div class="form-group" style="flex:1;">
        ...
        <input type="text" class="form-control" id="config-liveStartTime" placeholder="예: 오늘 밤 9시 시작" value="${t.liveStartTime||``}">
      </div>
    </div>
