...
설정을 저장한 후 라이브를 시작할 수 있습니다.`),document.getElementById(`cfg-stream`).focus(),document.getElementById(`cfg-stream`).style.borderColor=`#ef4444`,document.getElementById(`cfg-stream`).style.boxShadow=`0 0 0 3px rgba(239,68,68,0.15)`,setTimeout(()=>{document.getElementById(`cfg-stream`).style.borderColor=``,document.getElementById(`cfg-stream`).style.boxShadow=``},3e3);return}if(u.isLive=!u.isLive,u.isLive){u.liveStartTime=new Date().toISOString();let e=document.getElementById(`cfg-liveStartTime`);e&&(e.value=new Date(new Date().getTime()-new Date().getTimezoneOffset()*6e4).toISOString().slice(0,16))}n.textContent=u.isLive?`라이브 종료`:`라이브 시작`,n.className=`action-btn ${u.isLive?`btn-danger-solid`:`btn-success-solid`}`,n.style.cssText=`flex:1; justify-content:center; padding:14px; font-size:15px;`;let i=w.querySelector(`div:nth-child(2)`);if(i){let e=i.querySelector(`span:last-child`);e&&(e.textContent.includes(`라이브 중`)||e.textContent.includes(`송출 대기`))&&(u.isLive?(e.innerHTML=`<span style="width:5px; height:5px; background:#ef4444; border-radius:50%; display:inline-block;"></span>라이브 중`,e.style.cssText=`font-size:10px; font-weight:800; color:#ef4444; background:#fee2e2; border:1px solid #fecaca; padding:2px 6px; border-radius:4px; white-space:nowrap; height:16px; display:inline-flex; align-items:center; gap:4px;`):(e.innerHTML=`송출 대기`,e.style.cssText=`font-size:10px; font-weight:800; color:#64748b; background:#f1f5f9; border:1px solid #e2e8f0; padding:2px 6px; border-radius:4px; white-space:nowrap; height:16px; display:inline-flex; align-items:center;`))}if(u.isLive){if(!document.getElementById(`onair-timer-wrapper`)){let e=document.createElement(`div`);e.id=`onair-timer-wrapper`,e.style.cssText=`display:inline-flex; align-items:center; gap:6px; font-size:12px; font-weight:700; color:#ef4444; background:#fef2f2; padding:4px 10px; border-radius:6px; border:1px solid #fecaca; white-space:nowrap;`,e.innerHTML=`<div style="width:6px; height:6px; background:#ef4444; border-radius:50%; box-shadow:0 0 0 2px #fee2e2;"></div> 방송 중 <span id="onair-timer-text" style="font-family:monospace; margin-left:2px; letter-spacing:0.02em;">00:00:00</span>`,w.appendChild(e)}let e=new Date(u.liveStartTime).getTime();k();let t=document.getElementById(`onair-timer-text`);if(t){let n=()=>{let n=Date.now()-e,r=Math.floor(n/36e5),i=Math.floor(n%36e5/6e4),a=Math.floor(n%6e4/1e3);t.textContent=`${r.toString().padStart(2,`0`)}:${i.toString().padStart(2,`0`)}:${a.toString().padStart(2,`0`)}`};n(),O=setInterval(n,1e3)}}else{k();let e=document.getElementById(`onair-timer-wrapper`);e&&e.remove()}v(),jt(t,u,d,f,!0)});let h=async(e,n,r)=>{if(e){document.getElementById(n).style.opacity=`0.5`;try{let i=r===`logoUrl`,a=await c(await l(e,i?256:1080,i?256:1920,i?.88:.82));u[r]=a;let o=document.getElementById(n);if(o.src=a,o.style.display=`block`,r===`shareImageUrl`){let e=document.getElementById(`og-preview-img`),t=document.getElementById(`og-preview-img-placeholder`),n=document.getElementById(`og-img-placeholder`);e&&(e.src=a,e.style.display=`block`),t&&(t.style.display=`none`),n&&(n.style.display=`none`)}else if(r===`standbyImageUrl`){u.useStandbyImage=!0;let e=document.getElementById(`cfg-useStandbyImage`);e&&(e.checked=!0);let t=document.getElementById(`cfg-useStandbyImage-label`);t&&(t.textContent=`ON`,t.style.color=`#2563eb`);let n=document.getElementById(`standby-image-placeholder`);n&&(n.style.display=`none`);let r=document.getElementById(`btn-clear-standby-image`);r&&(r.style.display=`block`)}v(),jt(t,u,d,f,!0)}catch(e){console.error(`이미지 업로드 오류:`,e),alert(`이미지 업로드 실패: `+e.message)}finally{document.getElementById(n).style.opacity=`1`}}},g=async e=>{if(!e)return;let t=document.getElementById(`like-preview`),n=document.getElementById(`like-preview-placeholder`),r=document.getElementById(`btn-clear-like-icon`);t.style.opacity=`0.5`;try{let i=``;i=e.size<1.2*1024*1024?await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result.split(`,`)[1]),n.readAsDataURL(e)}):await l(e,512,512,.9);let a=await c(i);u.likeImageUrl=a,t.src=a,t.style.display=`block`,n&&(n.style.display=`none`),r&&(r.style.display=`block`),v()}catch(e){console.error(`응원 이미지 업로드 오류:`,e),alert(`응원 이미지 업로드 실패: `+e.message)}finally{t.style.opacity=`1`}};document.getElementById(`cfg-logoFile`).addEventListener(`change`,e=>h(e.target.files[0],`logo-preview`,`logoUrl`)),document.getElementById(`cfg-thumbnailFile`).addEventListener(`change`,e=>h(e.target.files[0],`thumbnail-preview`,`thumbnailUrl`)),document.getElementById(`cfg-shareImageFile`).addEventListener(`change`,e=>h(e.target.files[0],`share-image-preview`,`shareImageUrl`)),document.getElementById(`cfg-likeFile`).addEventListener(`change`,e=>g(e.target.files[0])),document.getElementById(`cfg-standbyImageFile`).addEventListener(`change`,async e=>{let t=e.target.files[0];if(!t)return;await h(t,`standby-image-preview`,`standbyImageUrl`),u.useStandbyImage=!0,v();let n=document.getElementById(`cfg-useStandbyImage`);n&&(n.checked=!0);let r=document.getElementById(`cfg-useStandbyImage-label`);r&&(r.textContent=`ON`,r.style.color=`#2563eb`);let i=document.getElementById(`standby-image-placeholder`);i&&(i.style.display=`none`);let a=document.getElementById(`btn-clear-standby-image`);a&&(a.style.display=`block`)});let _=document.getElementById(`btn-clear-standby-image`);_&&_.addEventListener(`click`,()=>{u.standbyImageUrl=``,u.useStandbyImage=!1,v(),le()});let x=document.getElementById(`cfg-useStandbyImage`);x&&x.addEventListener(`change`,e=>{u.useStandbyImage=e.target.checked,v();let t=document.getElementById(`cfg-useStandbyImage-label`);t&&(t.textContent=u.useStandbyImage?`ON`:`OFF`,t.style.color=u.useStandbyImage?`#2563eb`:`#94a3b8`);let n=document.getElementById(`live-preview-iframe`);n&&(n.src=M+`?t=`+Date.now())}),document.getElementById(`btn-clear-like-icon`).addEventListener(`click`,()=>{u.likeImageUrl=``;let e=document.getElementById(`like-preview`),t=document.getElementById(`like-preview-placeholder`),n=document.getElementById(`btn-clear-like-icon`);e&&(e.src=``,e.style.display=`none`),t&&(t.style.display=`block`),n&&(n.style.display=`none`),v()}),X&&X.from(`live_control`).select(`*`).eq(`live_id`,t).maybeSingle().then(({data:e,error:t})=>{if(t)throw t;if(e){let t=parseInt(e.cum_viewers)||0,n=parseInt(e.viewers)||0,r=parseInt(e.hearts)||0,i=document.getElementById(`cfg-cumViewers`);i&&(i.value=t);let a=document.getElementById(`cfg-viewers`);a&&!a.matches(`:focus`)&&(a.value=n);let o=document.getElementById(`cfg-hearts`);o&&!o.matches(`:focus`)&&(o.value=r),d.cumViewers=t,d.viewers=n,d.hearts=r,y()}}).catch(e=>console.warn(`Failed to fetch stats from Supabase`,e))},B=()=>{A.innerHTML=`
    ...
    `;let e=document.getElementById(`cfg-scheme`);e&&(e.value=u.scheme||``);let t=document.getElementById(`btn-save-scheme`);t&&t.addEventListener(`click`,async()=>{t.disabled=!0,t.textContent=`저장 중...`;try{u.scheme=e.value,v(),alert(`스킴 정보가 정상적으로 저장되었습니다.`)}catch(e){console.error(`스킴 저장 실패:`,e),alert(`스킴 저장 중 오류가 발생했습니다.`)}finally{t.disabled=!1,t.textContent=`스킴 저장`}})},ue=()=>{A.innerHTML=`
      <!-- 서브 탭 네비게이션 -->
...
`+e.message)}finally{a&&(a.style.opacity=`1`)}})}),n.querySelectorAll(`.btn-deal-start`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx),a=parseInt(document.getElementById(`deal-min-${i}`).value);a>0&&(f[i].dealText=document.getElementById(`deal-text-${i}`).value||`깜짝딜 종료까지`,f[i].dealEndTime=Date.now()+a*60*1e3,b(),n.innerHTML=de(),e(),jt(t,u,d,f,!0),setTimeout(()=>alert(`${a}분 깜짝딜이 시작되었습니다!`),10))})}),n.querySelectorAll(`.btn-deal-cancel`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx);f[i].dealEndTime=0,b(),n.innerHTML=de(),e(),jt(t,u,d,f,!0)})}),n.querySelectorAll(`.btn-del-product`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.currentTarget.dataset.idx),i=f[r]?.name||`이 상품`;confirm(`정말 "${i}" 상품을 삭제하시겠습니까?`)&&(f.splice(r,1),b(!0),n.innerHTML=de(),e())})}),n.querySelectorAll(`.btn-move-up`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.currentTarget.dataset.idx);if(r>0){let t=f[r-1];f[r-1]=f[r],f[r]=t,b(!0),n.innerHTML=de(),e()}})}),n.querySelectorAll(`.btn-move-down`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.currentTarget.dataset.idx);if(r<f.length-1){let t=f[r+1];f[r+1]=f[r],f[r]=t,b(!0),n.innerHTML=de(),e()}})})};e(),document.getElementById(`btn-add-product`).addEventListener(`click`,()=>{f.push({id:Date.now(),name:`새 상품`,price:``,normalPrice:``,discountRate:0,image:`https://via.placeholder.com/72`,url:`#`}),b(!0),document.getElementById(`product-list-container`).innerHTML=de(),e()}),document.getElementById(`btn-save-products`).addEventListener(`click`,()=>{jt(t,u,d,f,!0),alert(`상품 목록이 적용되었습니다!`)})},pe=()=>{A.innerHTML=`
      <div class="section-card">
        <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom:16px; border-bottom:1.5px solid #f1f5f9; margin-bottom:20px;">
