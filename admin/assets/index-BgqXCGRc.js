...
설정을 저장한 후 라이브를 시작할 수 있습니다.`),document.getElementById(`cfg-stream`).focus(),document.getElementById(`cfg-stream`).style.borderColor=`#ef4444`,document.getElementById(`cfg-stream`).style.boxShadow=`0 0 0 3px rgba(239,68,68,0.15)`,setTimeout(()=>{document.getElementById(`cfg-stream`).style.borderColor=``,document.getElementById(`cfg-stream`).style.boxShadow=``},3e3);return}r.isLive=!r.isLive,e.target.textContent=r.isLive?`라이브 종료`:`라이브 시작`,e.target.className=`action-btn ${r.isLive?`btn-danger-solid`:`btn-success-solid`}`,e.target.style.cssText=`flex:1; justify-content:center; padding:14px; font-size:15px;`,s(),st(t,r,i,a,!0)});let e=async(e,t,n)=>{if(!e)return;let i=new FormData;i.append(`file`,e),document.getElementById(t).style.opacity=`0.5`;try{let e=await(await fetch(`https://tmpfiles.org/api/v1/upload`,{method:`POST`,body:i})).json();if(e.status===`success`){let i=e.data.url.replace(`tmpfiles.org/`,`tmpfiles.org/dl/`);r[n]=i,document.getElementById(t).src=i,s()}}catch(e){console.error(e)}finally{document.getElementById(t).style.opacity=`1`}};document.getElementById(`cfg-logoFile`).addEventListener(`change`,t=>e(t.target.files[0],`logo-preview`,`logoUrl`)),document.getElementById(`cfg-thumbnailFile`).addEventListener(`change`,t=>e(t.target.files[0],`thumbnail-preview`,`thumbnailUrl`)),fetch(`${Je}?sheet=%EB%9D%BC%EC%9D%B4%EB%B8%8C%EA%B4%80%EC%A0%9C`).then(e=>e.json()).then(e=>{if(Array.isArray(e)&&e.length>0){let n=e.find(e=>e.live_id===t);if(n){let e=parseInt(n.누적시청자수)||0,t=parseInt(n.시청자수)||0,r=parseInt(n.하트수)||0,a=document.getElementById(`cfg-cumViewers`);a&&(a.value=e);let o=document.getElementById(`cfg-viewers`);o&&!o.matches(`:focus`)&&(o.value=t);let s=document.getElementById(`cfg-hearts`);s&&!s.matches(`:focus`)&&(s.value=r),i.cumViewers=e,i.viewers=t,i.hearts=r,c()}}}).catch(e=>console.warn(`Failed to fetch stats from 라이브관제`,e))},b=()=>{m.innerHTML=`
      <div class="section-card">
        <h3>관리자 채팅 발송</h3>
