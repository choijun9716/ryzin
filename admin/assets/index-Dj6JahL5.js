...
설정을 저장한 후 라이브를 시작할 수 있습니다.`),document.getElementById(`cfg-stream`).focus(),document.getElementById(`cfg-stream`).style.borderColor=`#ef4444`,document.getElementById(`cfg-stream`).style.boxShadow=`0 0 0 3px rgba(239,68,68,0.15)`,setTimeout(()=>{document.getElementById(`cfg-stream`).style.borderColor=``,document.getElementById(`cfg-stream`).style.boxShadow=``},3e3);return}r.isLive=!r.isLive,e.target.textContent=r.isLive?`라이브 종료`:`라이브 시작`,e.target.className=`action-btn ${r.isLive?`btn-danger-solid`:`btn-success-solid`}`,e.target.style.cssText=`flex:1; justify-content:center; padding:14px; font-size:15px;`,s(),st(t,r,i,a,!0)});let e=async(e,t,n)=>{if(e){document.getElementById(t).style.opacity=`0.5`;try{let i=await new Promise((t,n)=>{let r=new FileReader;r.onload=()=>t(r.result.split(`,`)[1]),r.onerror=n,r.readAsDataURL(e)}),a=new FormData;a.append(`key`,`117dfb947bc9e0045774b193d1eef7b6`),a.append(`image`,i);let o=await(await fetch(`https://api.imgbb.com/1/upload`,{method:`POST`,body:a})).json();if(o.success){let e=o.data.url;r[n]=e,document.getElementById(t).src=e,s()}else console.error(`ImgBB 업로드 실패:`,o)}catch(e){console.error(`이미지 업로드 오류:`,e)}finally{document.getElementById(t).style.opacity=`1`}}};document.getElementById(`cfg-logoFile`).addEventListener(`change`,t=>e(t.target.files[0],`logo-preview`,`logoUrl`)),document.getElementById(`cfg-thumbnailFile`).addEventListener(`change`,t=>e(t.target.files[0],`thumbnail-preview`,`thumbnailUrl`)),Z&&Z.from(`live_control`).select(`*`).eq(`live_id`,t).maybeSingle().then(({data:e,error:t})=>{if(t)throw t;if(e){let t=parseInt(e.cum_viewers)||0,n=parseInt(e.viewers)||0,r=parseInt(e.hearts)||0,a=document.getElementById(`cfg-cumViewers`);a&&(a.value=t);let o=document.getElementById(`cfg-viewers`);o&&!o.matches(`:focus`)&&(o.value=n);let s=document.getElementById(`cfg-hearts`);s&&!s.matches(`:focus`)&&(s.value=r),i.cumViewers=t,i.viewers=n,i.hearts=r,c()}}).catch(e=>console.warn(`Failed to fetch stats from Supabase`,e))},b=()=>{m.innerHTML=`
      <div class="section-card">
        <h3>관리자 채팅 발송</h3>
