...
`;e.forEach(e=>{let t=new Date(e.created_at).toLocaleString(`ko-KR`).replace(/,/g,``),r=(e.name||``).replace(/,/g,` `),i=(e.phone||``).replace(/,/g,` `);n+=`${t},${r},${i}\n`});let r=new Blob([`﻿`+n],{type:`text/csv;charset=utf-8;`}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=`상담DB_${t}.csv`,document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(i)})},fe=S.querySelector(`#btn-back`);fe&&fe.addEventListener(`click`,()=>{A.dispatchEvent(new Event(`adminTabLeave`)),k(),m&&clearInterval(m),he&&X.removeChannel(he),me&&X.removeChannel(me),n(null)});let pe=S.querySelector(`#btn-refresh-preview`);pe&&pe.addEventListener(`click`,()=>{let e=S.querySelector(`#live-preview-iframe`);e&&(e.src=M)});let me=null;X&&(me=X.channel(`bot-sync-${t}`).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`live_chats`,filter:`live_id=eq.${t}`},e=>{let n=e.new;if(n&&p.autoReplyActive&&p.autoReplyRules&&p.autoReplyRules.length>0){let e=n.nickname||``;if(!e.includes(`|`)&&e!==`관리자`&&e!==`자동응답봇`){let e=(n.content||``).toLowerCase();for(let n of p.autoReplyRules)if(n.keywords.split(`,`).map(e=>e.trim().toLowerCase()).filter(e=>e).some(t=>e.includes(t))){setTimeout(async()=>{try{if(!X)return;await X.from(`live_chats`).insert([{live_id:t,nickname:`자동응답봇`,content:n.answer,created_at:Date.now().toString()}])}catch(e){console.warn(`Auto-reply failed`,e)}},600);break}}}}).subscribe());let he=null;X&&(he=X.channel(`admin-sync-${t}`).on(`postgres_changes`,{event:`UPDATE`,schema:`public`,table:`live_control`,filter:`live_id=eq.${t}`},e=>{let n=e.new;if(!n)return;u.brandName=n.title||``,u.title=n.subtitle||``;let r=n.profile_image||``,i=`라이브 보기`,a=`right`,o=``,s=!1,c=r.split(`#`),l=c[0];c.slice(1).forEach(e=>{e===`nosplash`||(e.startsWith(`widgetText=`)?i=decodeURIComponent(e.replace(`widgetText=`,``)):e.startsWith(`widgetPosition=`)?a=e.replace(`widgetPosition=`,``):e.startsWith(`widgetImageUrl=`)?o=e.replace(`widgetImageUrl=`,``):e.startsWith(`showOnMain=`)&&(s=e.replace(`showOnMain=`,``)===`true`))}),u.showSplash=!r.includes(`#nosplash`),u.logoUrl=l,u.widgetText=i,u.widgetPosition=a,u.widgetImageUrl=o,u.showOnMain=s,u.streamUrl=n.stream_url||``,u.showViewers=n.show_viewers!==!1,u.thumbnailUrl=n.thumbnail_url||``,u.liveStartTime=n.start_time||``,u.isLive=n.status===`ON`,u.shareTitle=n.share_title||``,u.shareDesc=n.share_desc||``,u.shareImageUrl=n.share_image||``,u.likeImageUrl=n.like_image_url||``,u.bannedWords=n.banned_words||``,u.bannedUsers=n.banned_users||``,n.winner_name!==void 0&&(u.winner_name=n.winner_name),n.winner_timestamp!==void 0&&(u.winner_timestamp=n.winner_timestamp),n.viewers!==void 0&&(d.viewers=parseInt(n.viewers)||0),n.cum_viewers!==void 0&&(d.cumViewers=parseInt(n.cum_viewers)||0),n.hearts!==void 0&&(d.hearts=parseInt(n.hearts)||0),St(t,u),wt(t,d),typeof window.updateAdminViewersDisplay==`function`&&window.updateAdminViewersDisplay();let p=(e,t)=>{let n=S.querySelector(`#`+e)||document.getElementById(e);n&&document.activeElement!==n&&(n.type===`checkbox`?n.checked=!!t:n.value=t)};p(`cfg-brandName`,u.brandName),p(`cfg-title`,u.title),p(`cfg-stream`,u.streamUrl),p(`cfg-showViewers`,u.showViewers),p(`cfg-liveStartTime`,u.liveStartTime),p(`cfg-shareTitle`,u.shareTitle),p(`cfg-shareDesc`,u.shareDesc),p(`cfg-bannedWords`,u.bannedWords),p(`cfg-bannedUsers`,u.bannedUsers);let m=S.querySelector(`#logo-preview`)||document.getElementById(`logo-preview`);m&&(m.src=u.logoUrl);let h=S.querySelector(`#thumbnail-preview`)||document.getElementById(`thumbnail-preview`);h&&(h.src=u.thumbnailUrl);let g=S.querySelector(`#like-preview`)||document.getElementById(`like-preview`);g&&(g.src=u.likeImageUrl,g.style.display=u.likeImageUrl?`block`:`none`);let _=S.querySelector(`#like-preview-placeholder`)||document.getElementById(`like-preview-placeholder`);_&&(_.style.display=u.likeImageUrl?`none`:`block`);let v=S.querySelector(`#btn-clear-like-icon`)||document.getElementById(`btn-clear-like-icon`);v&&(v.style.display=u.likeImageUrl?`block`:`none`);let y=S.querySelector(`#btn-toggle-live`)||document.getElementById(`btn-toggle-live`);if(y&&document.activeElement!==y&&(y.textContent=u.isLive?`라이브 종료`:`라이브 시작`,y.className=`action-btn ${u.isLive?`btn-danger-solid`:`btn-success-solid`}`),n.products&&Array.isArray(n.products)){let e=JSON.stringify(n.products);if(JSON.stringify(f)!==e){Array.isArray(f)||(f=[]),f.length=0,f.push(...n.products),Et(t,f);let e=S.querySelector(`#product-list-container`)||document.getElementById(`product-list-container`);e&&!e.contains(document.activeElement)&&typeof V==`function`&&(e.innerHTML=V())}}}).subscribe()),X&&setInterval(async()=>{try{let{data:e,error:n}=await X.from(`live_control`).select(`viewers, cum_viewers, hearts`).eq(`live_id`,t).maybeSingle();e&&!n&&(d.viewers=parseInt(e.viewers)||0,d.cumViewers=parseInt(e.cum_viewers)||0,d.hearts=parseInt(e.hearts)||0,wt(t,d),typeof window.updateAdminViewersDisplay==`function`&&window.updateAdminViewersDisplay())}catch{}},2500);let ge=w.querySelectorAll(`.tab-btn`),_e=e=>{A.dispatchEvent(new Event(`adminTabLeave`)),ge.forEach(t=>t.classList.toggle(`active`,t.dataset.tab===e)),e===`config`?le():e===`chat`?B():e===`product`?ue():e===`leads`&&de()};ge.forEach(e=>{e.addEventListener(`click`,()=>_e(e.dataset.tab))}),o?B():le()}function It(){let e=document.createElement(`div`),t=``;function n(){let r=W.getAll(`hosts`);if(t){let e=t.toLowerCase();r=r.filter(t=>t.name.toLowerCase().includes(e)||t.phone&&t.phone.includes(e))}let i=r.map(e=>{let t=W.getHostStats(e.id);return{...e,stats:t}});e.innerHTML=`
    ...
    `,setTimeout(()=>{e.querySelector(`#host-search`)?.addEventListener(`input`,e=>{t=e.target.value,n();let r=document.getElementById(`host-search`);if(r){r.focus();let e=r.value.length;r.setSelectionRange(e,e)}}),e.querySelector(`#btn-add-host`)?.addEventListener(`click`,()=>{Lt()}),e.querySelectorAll(`.host-link`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault(),M.navigate(`/hosts/${e.getAttribute(`data-id`)}`)})}),e.querySelectorAll(`.btn-edit-host`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),Lt(e.getAttribute(`data-id`))})}),e.querySelectorAll(`tr.clickable`).forEach(e=>{e.addEventListener(`click`,()=>{M.navigate(`/hosts/${e.getAttribute(`data-id`)}`)})})},0)}return n(),W.on(`hosts:changed`,n),e}function Lt(e=null){let t=!!e,n=t?W.getById(`hosts`,e):{},r=`
    <div class="form-grid">
