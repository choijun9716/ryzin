...
다시 원래의 실제 운영 데이터를 불러옵니다.`,confirmText:`복귀하기`,onConfirm:()=>W.toggleDemoMode(!1),onCancel:()=>{n.checked=!0}})}),t===`admin`){let t=JSON.parse(localStorage.getItem(`ryzin_live_data`)||`{}`),n=t.settings||{},r=e.querySelector(`#setting-popbill-linkid`),i=e.querySelector(`#setting-popbill-secret`),a=e.querySelector(`#setting-popbill-sender`);r&&(r.value=n.popbillLinkId||``),i&&(i.value=n.popbillSecretKey||``),a&&(a.value=n.popbillSenderNumber||``),e.querySelector(`#btn-save-api-settings`)?.addEventListener(`click`,()=>{t.settings||={},t.settings.popbillLinkId=r.value.trim(),t.settings.popbillSecretKey=i.value.trim(),t.settings.popbillSenderNumber=a.value.trim(),localStorage.setItem(`ryzin_live_data`,JSON.stringify(t)),J(`API 연동 설정이 저장되었습니다.`)})}t===`admin`&&(Jn(e),e.querySelector(`#btn-create-user`)?.addEventListener(`click`,async()=>{await Yn()}))},0),e}function Jn(e){let t=e.querySelector(`#user-list-tbody`);if(!t)return;let n=W.getAll(`users`);if(n.length===0){t.innerHTML=`<tr><td colspan="5" class="text-center" style="padding: var(--space-8); color: var(--text-tertiary);">등록된 사용자가 없습니다.</td></tr>`;return}t.innerHTML=n.map(e=>{let t=e.role;if(ae[e.role])t=ae[e.role].label;else if(e.role&&e.role.startsWith(`live_stream:`))t=`송출 관리자 (${e.role.split(`:`)[1]})`;else if(e.role&&e.role.startsWith(`brand:`)){let n=e.role.split(`:`)[1],r=W.getById(`brands`,n);t=r?`파트너사 (${r.name})`:`파트너사 (${n})`}return`
  ...
  `,e.querySelector(`#add-user-btn`).addEventListener(`click`,()=>{Fa(null,t,e)}),e.querySelectorAll(`.user-edit-btn`).forEach(r=>{r.addEventListener(`click`,()=>{Fa(n.find(e=>e.id===r.dataset.id),t,e)})})}function Fa(e,t,n){let r=!!e,i=t.querySelector(`#sm-modal-container`),a=e?e.user_code:`USER-${Math.floor(Math.random()*89999+1e4)}`,o=e&&e.phone||``,s=e&&e.email||``;!o&&s&&(s.startsWith(`01`)||s.includes(`-`)||!s.includes(`@`))&&(o=s.replace(`@kakao.user`,``),s=``),i.innerHTML=`
    <div class="sm-modal-backdrop">
