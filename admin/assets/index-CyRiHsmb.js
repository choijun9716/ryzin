...
`).map(e=>e.trim()).filter(e=>e.includes(`|`));if(a.length===0){alert(`닉네임|내용 형식으로 1줄 이상 입력해주세요.`),h=!1;return}e.textContent=`⏸`,r.textContent=`채팅 봇 중지`,i.className=`action-btn btn-danger-solid`,i.style.cssText=`width:100%; justify-content:center; padding:14px; font-size:15px; gap:8px;`;let o=parseInt(f.value)||10;m=setInterval(async()=>{let[e,...r]=a[Math.floor(Math.random()*a.length)].split(`|`),i=r.join(`|`).trim();if(!e||!i)return;let o=Date.now(),s=document.createElement(`div`);s.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`,s.innerHTML=`<span style="font-weight:700; color:#64748b;">${e.trim()}:</span> ${i}`,n.innerHTML.includes(`실시간 채팅`)&&(n.innerHTML=``),n.appendChild(s),n.scrollTop=n.scrollHeight;try{if(!Z)return;await Z.from(`live_chats`).insert([{live_id:t,created_at:o,nickname:e.trim(),content:i}])}catch(e){console.warn(`Bot chat failed`,e)}},o*1e3)}else m&&clearInterval(m),e.textContent=`▶`,r.textContent=`채팅 봇 시작`,i.className=`action-btn btn-primary-solid`,i.style.cssText=`width:100%; justify-content:center; padding:14px; font-size:15px; gap:8px;`});let g=0,_=null,v=(e,t,r=!1)=>{n.innerHTML.includes(`실시간 채팅 내역이 여기에`)&&(n.innerHTML=``);let i=document.createElement(`div`);i.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`+(r?`opacity:0.72;`:``),i.innerHTML=`<span style="font-weight:700; color:${e===`관리자`?`#3b82f6`:`#64748b`};">${e}:</span> ${t}`,n.appendChild(i),r||(n.scrollTop=n.scrollHeight)};(async()=>{if(Z)try{let{data:e,error:r}=await Z.from(`live_chats`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!0}).limit(100);if(r)throw r;e&&Array.isArray(e)&&(e.forEach(e=>{v(e.nickname||`?`,e.content||``,!0),g=parseInt(e.created_at)||0}),setTimeout(()=>{n.scrollTop=n.scrollHeight},100))}catch(e){console.warn(`Failed to load chat history`,e)}})(),Z&&(_=Z.channel(`admin-chat-channel-${t}`).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`live_chats`,filter:`live_id=eq.${t}`},e=>{let t=e.new;t&&parseInt(t.created_at)>g&&(v(t.nickname||`?`,t.content||``,!1),g=parseInt(t.created_at))}).subscribe()),x.addEventListener(`adminTabLeave`,()=>{_&&Z.removeChannel(_),m&&clearInterval(m)})},A=()=>c.map((e,t)=>{let n=e.clicks||0;return`
