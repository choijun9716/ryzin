...
설정을 저장한 후 라이브를 시작할 수 있습니다.`),document.getElementById(`cfg-stream`).focus(),document.getElementById(`cfg-stream`).style.borderColor=`#ef4444`,document.getElementById(`cfg-stream`).style.boxShadow=`0 0 0 3px rgba(239,68,68,0.15)`,setTimeout(()=>{document.getElementById(`cfg-stream`).style.borderColor=``,document.getElementById(`cfg-stream`).style.boxShadow=``},3e3);return}if(o.isLive=!o.isLive,o.isLive){o.liveStartTime=new Date().toISOString();let e=document.getElementById(`cfg-liveStartTime`);e&&(e.value=new Date(new Date().getTime()-new Date().getTimezoneOffset()*6e4).toISOString().slice(0,16))}e.target.textContent=o.isLive?`라이브 종료`:`라이브 시작`,e.target.className=`action-btn ${o.isLive?`btn-danger-solid`:`btn-success-solid`}`,e.target.style.cssText=`flex:1; justify-content:center; padding:14px; font-size:15px;`,u(),st(t,o,s,c,!0)});let e=async(e,t,n)=>{if(e){document.getElementById(t).style.opacity=`0.5`;try{let r=n===`logoUrl`,s=await i(await a(e,r?256:1080,r?256:1920,r?.88:.82));o[n]=s,document.getElementById(t).src=s,u()}catch(e){console.error(`이미지 업로드 오류:`,e),alert(`이미지 업로드 실패: `+e.message)}finally{document.getElementById(t).style.opacity=`1`}}},n=async e=>{if(!e)return;let t=document.getElementById(`like-preview`),n=document.getElementById(`like-preview-placeholder`),r=document.getElementById(`btn-clear-like-icon`);t.style.opacity=`0.5`;try{let s=``;s=e.size<1.2*1024*1024?await new Promise(t=>{let n=new FileReader;n.onload=e=>t(e.target.result.split(`,`)[1]),n.readAsDataURL(e)}):await a(e,512,512,.9);let c=await i(s);o.likeImageUrl=c,t.src=c,t.style.display=`block`,n&&(n.style.display=`none`),r&&(r.style.display=`block`),u()}catch(e){console.error(`응원 이미지 업로드 오류:`,e),alert(`응원 이미지 업로드 실패: `+e.message)}finally{t.style.opacity=`1`}};document.getElementById(`cfg-logoFile`).addEventListener(`change`,t=>e(t.target.files[0],`logo-preview`,`logoUrl`)),document.getElementById(`cfg-thumbnailFile`).addEventListener(`change`,t=>e(t.target.files[0],`thumbnail-preview`,`thumbnailUrl`)),document.getElementById(`cfg-shareImageFile`).addEventListener(`change`,t=>e(t.target.files[0],`share-image-preview`,`shareImageUrl`)),document.getElementById(`cfg-likeFile`).addEventListener(`change`,e=>n(e.target.files[0])),document.getElementById(`btn-clear-like-icon`).addEventListener(`click`,()=>{o.likeImageUrl=``;let e=document.getElementById(`like-preview`),t=document.getElementById(`like-preview-placeholder`),n=document.getElementById(`btn-clear-like-icon`);e&&(e.src=``,e.style.display=`none`),t&&(t.style.display=`block`),n&&(n.style.display=`none`),u()}),Z&&Z.from(`live_control`).select(`*`).eq(`live_id`,t).maybeSingle().then(({data:e,error:t})=>{if(t)throw t;if(e){let t=parseInt(e.cum_viewers)||0,n=parseInt(e.viewers)||0,r=parseInt(e.hearts)||0,i=document.getElementById(`cfg-cumViewers`);i&&(i.value=t);let a=document.getElementById(`cfg-viewers`);a&&!a.matches(`:focus`)&&(a.value=n);let o=document.getElementById(`cfg-hearts`);o&&!o.matches(`:focus`)&&(o.value=r),s.cumViewers=t,s.viewers=n,s.hearts=r,d()}}).catch(e=>console.warn(`Failed to fetch stats from Supabase`,e))},k=()=>{x.innerHTML=`
    ...
    `;let e=document.getElementById(`admin-chat-input`),n=document.getElementById(`admin-chat-list`),r=document.getElementById(`admin-nickname-input`),i=document.getElementById(`admin-color-input`),a=document.getElementById(`admin-color-code`),o=document.getElementById(`admin-bg-color-input`),s=document.getElementById(`admin-bg-color-code`),c=document.getElementById(`btn-announce-winner`);c&&c.addEventListener(`click`,async()=>{let e=document.getElementById(`winner-nickname-input`).value.trim();if(!e){alert(`소통왕 당첨자 닉네임을 입력해 주세요.`);return}c.disabled=!0,c.textContent=`발표 중...`;try{if(!Z)return;let{error:n}=await Z.from(`live_control`).update({winner_name:e,winner_timestamp:Date.now(),updated_at:new Date().toISOString()}).eq(`live_id`,t);if(n)throw n;alert(`🎉 [${e}]님을 소통왕 당첨자로 공포했습니다!\n시청자 화면에 실시간 폭죽 팝업이 출력됩니다.`)}catch(e){alert(`소통왕 발표에 실패했습니다: `+e.message)}finally{c.disabled=!1,c.textContent=`당첨자 발표`}});let u=async()=>{let e=document.getElementById(`winner-table-body`);if(e)try{if(!Z)return;let{data:n,error:r}=await Z.from(`live_winners`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!1});if(r)throw r;if(!n||n.length===0){e.innerHTML=`<tr><td colspan="5" style="text-align:center; padding:30px; color:#94a3b8;">당첨자 제출 목록이 없습니다.</td></tr>`;return}e.innerHTML=n.map(e=>{let t=new Date(e.created_at).toLocaleTimeString(`ko-KR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`});return`
          ...
          `}).join(``)}catch(e){console.warn(`Failed to fetch winners`,e)}};document.getElementById(`btn-refresh-winners`)?.addEventListener(`click`,u),u();let d=setInterval(u,1e4);r&&r.addEventListener(`input`,()=>{localStorage.setItem(`ryzin_admin_nickname`,r.value.trim())}),i&&i.addEventListener(`input`,()=>{localStorage.setItem(`ryzin_admin_color`,i.value),a&&(a.textContent=i.value)}),o&&o.addEventListener(`input`,()=>{localStorage.setItem(`ryzin_admin_bg_color`,o.value),s&&(s.textContent=o.value)});let f=!1,m=async()=>{let a=e.value.trim();if(!a||f)return;f=!0;let s=(r?r.value.trim():``)||`관리자`,c=i?i.value:`#ffca28`,l=o?o.value:`#e50914`,u=`${s}|${c}|${l}`,d=Date.now(),p=document.createElement(`div`);p.style.cssText=`margin-bottom:8px; padding:8px 12px; border-radius:10px; background:${l}22; border-left:4px solid ${c}; display:flex; flex-direction:column; gap:2px;`,p.innerHTML=`<span style="font-weight:700; color:${c}; font-size:12px;">${s}</span><span style="font-size:13px; color:#1e293b;">${a}</span>`,n.innerHTML.includes(`실시간 채팅`)&&(n.innerHTML=``),n.appendChild(p),n.scrollTop=n.scrollHeight,e.value=``;try{if(!Z)return;await Z.from(`live_chats`).insert([{live_id:t,created_at:d,nickname:u,content:a}])}catch(e){console.warn(`Admin chat send failed`,e)}finally{f=!1}};document.getElementById(`btn-send-chat`).addEventListener(`click`,m),e.addEventListener(`keypress`,e=>{e.key===`Enter`&&m()}),document.getElementById(`btn-clear-chats`).addEventListener(`click`,async()=>{if(confirm(`📡 이 라이브 방송의 모든 실시간 채팅 내역을 초기화(영구 삭제)하시겠습니까?
    ...
    `}).join(``),j=()=>{x.innerHTML=`
      ...
      <div class="section-card" style="margin-bottom: 24px; border: 1.5px solid #fab005; background: #fffdf5;">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
          <input type="text" class="modern-input" style="flex:2; padding:8px 12px; font-size:13px;" id="winner-announce-text" placeholder="당첨자 닉네임 입력 (예: 라이진)" value="${o.winner_name||``}">
...
`+e.message)}finally{o&&(o.style.opacity=`1`)}})}),n.querySelectorAll(`.btn-deal-start`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx),a=parseInt(document.getElementById(`deal-min-${i}`).value);a>0&&(c[i].dealText=document.getElementById(`deal-text-${i}`).value||`깜짝딜 종료까지`,c[i].dealEndTime=Date.now()+a*60*1e3,f(),n.innerHTML=A(),e(),st(t,o,s,c,!0),setTimeout(()=>alert(`${a}분 깜짝딜이 시작되었습니다!`),10))})}),n.querySelectorAll(`.btn-deal-cancel`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx);c[i].dealEndTime=0,f(),n.innerHTML=A(),e(),st(t,o,s,c,!0)})}),n.querySelectorAll(`.btn-del-product`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx);c.splice(r,1),f(),n.innerHTML=A(),e()})})};e(),document.getElementById(`btn-add-product`).addEventListener(`click`,()=>{c.push({id:Date.now(),name:`새 상품`,price:``,normalPrice:``,discountRate:0,image:`https://via.placeholder.com/72`,url:`#`}),f(),document.getElementById(`product-list-container`).innerHTML=A(),e()}),document.getElementById(`btn-save-products`).addEventListener(`click`,()=>{st(t,o,s,c,!0),alert(`상품 목록이 적용되었습니다!`)});let n=document.getElementById(`btn-winner-start`),r=document.getElementById(`btn-winner-cancel`);n&&n.addEventListener(`click`,async()=>{let e=document.getElementById(`winner-announce-text`).value.trim(),r=parseInt(document.getElementById(`winner-announce-min`).value)||1;if(!e){alert(`소통왕 당첨자 닉네임을 입력해 주세요.`);return}n.disabled=!0,n.textContent=`적용 중...`;let i=Date.now()+r*60*1e3;try{if(!Z)return;let{error:n}=await Z.from(`live_control`).update({winner_name:e,winner_timestamp:i,updated_at:new Date().toISOString()}).eq(`live_id`,t);if(n)throw n;o.winner_name=e,o.winner_timestamp=i,alert(`🎉 소통왕 당첨 배너 노출이 시작되었습니다!`),j()}catch{alert(`시작 처리에 실패했습니다.`)}finally{n.disabled=!1,n.textContent=`시작`}}),r&&r.addEventListener(`click`,async()=>{r.disabled=!0,r.textContent=`종료 중...`;try{if(!Z)return;let{error:e}=await Z.from(`live_control`).update({winner_timestamp:0,updated_at:new Date().toISOString()}).eq(`live_id`,t);if(e)throw e;o.winner_timestamp=0,alert(`소통왕 당첨 배너 노출이 종료되었습니다.`),j()}catch{alert(`종료 처리에 실패했습니다.`)}finally{r.disabled=!1,r.textContent=`종료`}})};setTimeout(()=>{document.getElementById(`btn-back`).addEventListener(`click`,()=>{b(),n(null)}),document.getElementById(`btn-refresh-preview`).addEventListener(`click`,()=>{document.getElementById(`live-preview-iframe`).src=C});let e=g.querySelectorAll(`.tab-btn`),t=t=>{e.forEach(e=>e.classList.toggle(`active`,e.dataset.tab===t)),t===`config`?O():t===`chat`?k():t===`product`&&j()};e.forEach(e=>{e.addEventListener(`click`,()=>t(e.dataset.tab))}),O()},0)}function ft(){let e=document.createElement(`div`),t=``;function n(){let r=H.getAll(`hosts`);if(t){let e=t.toLowerCase();r=r.filter(t=>t.name.toLowerCase().includes(e)||t.phone&&t.phone.includes(e))}let i=r.map(e=>{let t=H.getHostStats(e.id);return{...e,stats:t}});e.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
          <div>
