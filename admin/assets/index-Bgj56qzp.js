...
`+e.message)}finally{o&&(o.style.opacity=`1`)}})}),n.querySelectorAll(`.btn-deal-start`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx),a=parseInt(document.getElementById(`deal-min-${i}`).value);a>0&&(c[i].dealText=document.getElementById(`deal-text-${i}`).value||`깜짝딜 종료까지`,c[i].dealEndTime=Date.now()+a*60*1e3,g(),n.innerHTML=N(),e(),At(t,o,s,c,!0),setTimeout(()=>alert(`${a}분 깜짝딜이 시작되었습니다!`),10))})}),n.querySelectorAll(`.btn-deal-cancel`).forEach(r=>{r.addEventListener(`click`,r=>{let i=parseInt(r.target.dataset.idx);c[i].dealEndTime=0,g(),n.innerHTML=N(),e(),At(t,o,s,c,!0)})}),n.querySelectorAll(`.btn-del-product`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx),i=c[r]?.name||`이 상품`;confirm(`정말 "${i}" 상품을 삭제하시겠습니까?`)&&(c.splice(r,1),g(),n.innerHTML=N(),e())})}),n.querySelectorAll(`.btn-move-up`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx);if(r>0){let t=c[r-1];c[r-1]=c[r],c[r]=t,g(),n.innerHTML=N(),e()}})}),n.querySelectorAll(`.btn-move-down`).forEach(t=>{t.addEventListener(`click`,t=>{let r=parseInt(t.target.dataset.idx);if(r<c.length-1){let t=c[r+1];c[r+1]=c[r],c[r]=t,g(),n.innerHTML=N(),e()}})})};e(),document.getElementById(`btn-add-product`).addEventListener(`click`,()=>{c.push({id:Date.now(),name:`새 상품`,price:``,normalPrice:``,discountRate:0,image:`https://via.placeholder.com/72`,url:`#`}),g(),document.getElementById(`product-list-container`).innerHTML=N(),e()}),document.getElementById(`btn-save-products`).addEventListener(`click`,()=>{At(t,o,s,c,!0),alert(`상품 목록이 적용되었습니다!`)})},ee=()=>{w.innerHTML=`
    ...
    `;let e=[],n=async()=>{try{if(!Z)throw Error(`Supabase 미연동`);let{data:n,error:r}=await Z.from(`live_leads`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!1});if(r)throw r;e=n||[];let i=document.getElementById(`leads-total-count`);i&&(i.textContent=e.length);let a=document.getElementById(`leads-list-container`),o=document.getElementById(`btn-download-csv-leads`);if(o&&(o.style.display=e.length>0?`block`:`none`),!a)return;if(e.length===0){a.innerHTML=`<div style="text-align:center; padding:40px; color:#94a3b8; font-size:14px; background:#f8fafc; border-radius:12px;">아직 접수된 상담문의가 없습니다.</div>`;return}let s=`
