...
`;e.forEach(e=>{let t=new Date(e.created_at).toLocaleString(`ko-KR`).replace(/,/g,``),r=(e.name||``).replace(/,/g,` `),i=(e.phone||``).replace(/,/g,` `);n+=`${t},${r},${i}\n`});let r=new Blob([`﻿`+n],{type:`text/csv;charset=utf-8;`}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=`상담DB_${t}.csv`,document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(i)})},fe=()=>{A.innerHTML=`
    ...
    `;let e=[],n=async()=>{try{let n=[];if(X){try{let{data:e}=await X.from(`live_orders`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!1});Array.isArray(e)&&n.push(...e)}catch{}try{let{data:e}=await X.from(`live_winners`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!1});Array.isArray(e)&&e.forEach(e=>{if(e.nickname&&(e.nickname.startsWith(`{"type":"order"`)||e.nickname.startsWith(`{"type": "order"`)))try{let t=JSON.parse(e.nickname);n.push({id:e.id,live_id:e.live_id,customer_name:e.name,customer_phone:e.phone,customer_address:e.address,total_amount:t.total||0,items:t.items||[{name:t.goodname||`상품`,price:t.total||0}],payment_status:t.status||`payapp_requested`,pg_provider:t.pg_provider||`payapp`,pg_receipt_id:t.mul_no||``,created_at:e.created_at})}catch{}})}catch{}}let i=[];try{i=JSON.parse(localStorage.getItem(`ryzin_live_orders_${t}`)||`[]`)}catch{}let a=new Map;[...n,...i].forEach(e=>{let t=(e.pg_receipt_id&&e.pg_receipt_id!==`undefined`?e.pg_receipt_id:null)||e.id||e.created_at+`_`+e.customer_phone,n=[`cancelled`,`canceled`,`payapp_cancelled`,`refunded`,`cancel`],r=a.get(t);if(!r)a.set(t,e);else{let i=n.includes((e.payment_status||``).toLowerCase()),o=n.includes((r.payment_status||``).toLowerCase());i&&!o&&a.set(t,e)}}),e=Array.from(a.values()).sort((e,t)=>new Date(t.created_at||0)-new Date(e.created_at||0)),r();let o=e.map(e=>e.pg_receipt_id).filter(e=>e&&e!==`undefined`&&e!==`-`);if(o.length>0)try{let n=await fetch(`/api/payapp`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({cmd:`check_orders`,mul_nos:o})});if(n.ok){let i=await n.json();if(i.success&&i.results){let n=!1;if(e.forEach(e=>{let t=i.results[e.pg_receipt_id];t&&t.status&&t.status!==`unknown`&&(e.payment_status!==t.status||e.status_detail!==t.usingstatestr)&&(e.payment_status=t.status,e.status_detail=t.usingstatestr,n=!0)}),n){try{localStorage.setItem(`ryzin_live_orders_${t}`,JSON.stringify(e))}catch{}r()}}}}catch(e){console.warn(`PayApp 실시간 상태 동기화 중 오류 (일반 조회 계속 유지):`,e)}}catch(e){console.warn(`Failed to load orders`,e);let t=document.getElementById(`orders-list-container`);t&&(t.innerHTML=`<div style="text-align:center; padding:20px; color:#ef4444; font-size:13px;">주문 내역을 불러오는 데 실패했습니다.</div>`)}},r=()=>{let t=document.getElementById(`orders-list-container`),n=document.getElementById(`btn-download-csv-orders`),r=document.getElementById(`orders-count-badge`),i=document.getElementById(`stat-orders-count`),a=document.getElementById(`stat-orders-total`),o=document.getElementById(`order-search-input`),s=(o?o.value:``).trim().toLowerCase(),c=e;s&&(c=e.filter(e=>{let t=(e.customer_name||``).toLowerCase().includes(s),n=(e.customer_phone||``).includes(s),r=(e.customer_address||``).toLowerCase().includes(s),i=``;Array.isArray(e.items)?i=e.items.map(e=>e.name).join(` `):typeof e.items==`string`&&(i=e.items);let a=i.toLowerCase().includes(s);return t||n||r||a}));let l=[`cancelled`,`canceled`,`payapp_cancelled`,`refunded`,`cancel`],u=e.reduce((e,t)=>l.includes((t.payment_status||``).toLowerCase())?e:e+(Number(t.total_amount)||0),0);if(i&&(i.textContent=`${e.length.toLocaleString()}건`),a&&(a.textContent=`${u.toLocaleString()}원`),r&&(r.textContent=`${c.length}건`),n&&(n.style.display=e.length>0?`block`:`none`),!t)return;if(c.length===0){t.innerHTML=`<div style="text-align:center; padding:40px; color:#94a3b8; font-size:14px; background:#f8fafc; border-radius:12px;">주문 내역이 없습니다.</div>`;return}let d=`
        <div style="overflow-x:auto;">
          <table style="width:100%; border-collapse:collapse; text-align:left; font-size:13px; min-width:800px;">
            ...
            <tbody>
      `;c.forEach(e=>{let t=e.created_at?new Date(e.created_at).toLocaleString(`ko-KR`):`-`,n=`상품 정보 없음`;if(Array.isArray(e.items)&&e.items.length>0)n=e.items.length>1?`${e.items[0].name} 외 ${e.items.length-1}건`:e.items[0].name;else if(typeof e.items==`string`)try{let t=JSON.parse(e.items);Array.isArray(t)&&t.length>0&&(n=t.length>1?`${t[0].name} 외 ${t.length-1}건`:t[0].name)}catch{n=e.items}let r=Number(e.total_amount)||0,i=l.includes((e.payment_status||``).toLowerCase()),a=i?e.status_detail||`결제승인취소`:e.payment_status===`paid`?`결제완료`:e.status_detail||`결제요청`,o=i?`<span style="font-size:11px; font-weight:700; padding:2px 8px; border-radius:6px; background:#fef2f2; color:#dc2626; border:1px solid #fecaca;">${a}</span>`:e.payment_status===`paid`?`<span style="font-size:11px; font-weight:700; padding:2px 8px; border-radius:6px; background:#f0fdf4; color:#16a34a; border:1px solid #bbf7d0;">${a}</span>`:`<span style="font-size:11px; font-weight:700; padding:2px 8px; border-radius:6px; background:#eff6ff; color:#2563eb; border:1px solid #bfdbfe;">${a}</span>`;d+=`
          <tr style="border-bottom:1px solid #e2e8f0; ${i?`background:#fafafa;`:``} transition:background 0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='${i?`#fafafa`:`transparent`}'">
            ...
            <td style="${i?`padding:12px; font-weight:700; color:#94a3b8; text-align:right; font-family:monospace; text-decoration:line-through;`:`padding:12px; font-weight:800; color:#0f172a; text-align:right; font-family:monospace;`}">${r.toLocaleString()}원</td>
