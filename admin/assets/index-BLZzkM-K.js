...
설정을 저장한 후 라이브를 시작할 수 있습니다.`),document.getElementById(`cfg-stream`).focus(),document.getElementById(`cfg-stream`).style.borderColor=`#ef4444`,document.getElementById(`cfg-stream`).style.boxShadow=`0 0 0 3px rgba(239,68,68,0.15)`,setTimeout(()=>{document.getElementById(`cfg-stream`).style.borderColor=``,document.getElementById(`cfg-stream`).style.boxShadow=``},3e3);return}o.isLive=!o.isLive,e.target.textContent=o.isLive?`라이브 종료`:`라이브 시작`,e.target.className=`action-btn ${o.isLive?`btn-danger-solid`:`btn-success-solid`}`,e.target.style.cssText=`flex:1; justify-content:center; padding:14px; font-size:15px;`,u(),st(t,o,s,c,!0)});let e=async(e,t,n)=>{if(e){document.getElementById(t).style.opacity=`0.5`;try{let r=n===`logoUrl`,s=await i(await a(e,r?256:1080,r?256:1920,r?.88:.82));o[n]=s,document.getElementById(t).src=s,u()}catch(e){console.error(`이미지 업로드 오류:`,e),alert(`이미지 업로드 실패: `+e.message)}finally{document.getElementById(t).style.opacity=`1`}}};document.getElementById(`cfg-logoFile`).addEventListener(`change`,t=>e(t.target.files[0],`logo-preview`,`logoUrl`)),document.getElementById(`cfg-thumbnailFile`).addEventListener(`change`,t=>e(t.target.files[0],`thumbnail-preview`,`thumbnailUrl`)),document.getElementById(`cfg-shareImageFile`).addEventListener(`change`,t=>e(t.target.files[0],`share-image-preview`,`shareImageUrl`)),Z&&Z.from(`live_control`).select(`*`).eq(`live_id`,t).maybeSingle().then(({data:e,error:t})=>{if(t)throw t;if(e){let t=parseInt(e.cum_viewers)||0,n=parseInt(e.viewers)||0,r=parseInt(e.hearts)||0,i=document.getElementById(`cfg-cumViewers`);i&&(i.value=t);let a=document.getElementById(`cfg-viewers`);a&&!a.matches(`:focus`)&&(a.value=n);let o=document.getElementById(`cfg-hearts`);o&&!o.matches(`:focus`)&&(o.value=r),s.cumViewers=t,s.viewers=n,s.hearts=r,d()}}).catch(e=>console.warn(`Failed to fetch stats from Supabase`,e))},T=()=>{_.innerHTML=`
    ...
    `;let e=document.getElementById(`admin-chat-input`),n=document.getElementById(`admin-chat-list`),r=!1,i=async()=>{let i=e.value.trim();if(!i||r)return;r=!0;let a=Date.now(),o=document.createElement(`div`);o.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`,o.innerHTML=`<span style="font-weight:700; color:#3b82f6;">관리자:</span> ${i}`,n.innerHTML.includes(`실시간 채팅`)&&(n.innerHTML=``),n.appendChild(o),n.scrollTop=n.scrollHeight,e.value=``;try{if(!Z)return;await Z.from(`live_chats`).insert([{live_id:t,created_at:a,nickname:`관리자`,content:i}])}catch(e){console.warn(`Admin chat send failed`,e)}finally{r=!1}};document.getElementById(`btn-send-chat`).addEventListener(`click`,i),e.addEventListener(`keypress`,e=>{e.key===`Enter`&&i()}),document.getElementById(`btn-clear-chats`).addEventListener(`click`,async()=>{if(confirm(`📡 이 라이브 방송의 모든 실시간 채팅 내역을 초기화(영구 삭제)하시겠습니까?
이 작업은 복구할 수 없습니다.`)){let e=document.getElementById(`btn-clear-chats`);e.disabled=!0,e.textContent=`초기화 중...`;try{if(!Z)throw Error(`Supabase client가 로드되지 않았습니다.`);let{error:e}=await Z.from(`live_chats`).delete().eq(`live_id`,t);if(e)throw e;alert(`채팅 내역이 성공적으로 초기화되었습니다!`),n.innerHTML=`<div style="color:#94a3b8; text-align:center; padding-top:70px; font-weight:500;">
            <div style="font-size:24px; margin-bottom:8px;">💭</div>
            실시간 채팅 내역이 여기에 표시됩니다.
          </div>`}catch(e){alert(`채팅 내역 초기화 실패: `+e.message)}finally{e.disabled=!1,e.textContent=`채팅 내역 초기화`}}});let a=document.getElementById(`bot-chat-list`),o=document.getElementById(`bot-interval`);a.addEventListener(`input`,()=>{l.list=a.value,p()}),o.addEventListener(`input`,()=>{l.interval=parseInt(o.value)||10,p()});let s=null,c=!1;document.getElementById(`btn-toggle-bot`).addEventListener(`click`,()=>{c=!c;let e=document.getElementById(`bot-icon`),r=document.getElementById(`bot-text`),i=document.getElementById(`btn-toggle-bot`);if(c){let l=a.value.split(`
`).map(e=>e.trim()).filter(e=>e.includes(`|`));if(l.length===0){alert(`닉네임|내용 형식으로 1줄 이상 입력해주세요.`),c=!1;return}e.textContent=`⏸`,r.textContent=`채팅 봇 중지`,i.className=`action-btn btn-danger-solid`,i.style.cssText=`width:100%; justify-content:center; padding:14px; font-size:15px; gap:8px;`;let u=parseInt(o.value)||10;s=setInterval(async()=>{let[e,...r]=l[Math.floor(Math.random()*l.length)].split(`|`),i=r.join(`|`).trim();if(!e||!i)return;let a=Date.now(),o=document.createElement(`div`);o.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`,o.innerHTML=`<span style="font-weight:700; color:#64748b;">${e.trim()}:</span> ${i}`,n.innerHTML.includes(`실시간 채팅`)&&(n.innerHTML=``),n.appendChild(o),n.scrollTop=n.scrollHeight;try{if(!Z)return;await Z.from(`live_chats`).insert([{live_id:t,created_at:a,nickname:e.trim(),content:i}])}catch(e){console.warn(`Bot chat failed`,e)}},u*1e3)}else s&&clearInterval(s),e.textContent=`▶`,r.textContent=`채팅 봇 시작`,i.className=`action-btn btn-primary-solid`,i.style.cssText=`width:100%; justify-content:center; padding:14px; font-size:15px; gap:8px;`});let u=0,d=null,f=(e,t,r=!1)=>{n.innerHTML.includes(`실시간 채팅 내역이 여기에`)&&(n.innerHTML=``);let i=document.createElement(`div`);i.style.cssText=`margin-bottom:8px; padding:6px 0; border-bottom:1px solid #f1f5f9;`+(r?`opacity:0.72;`:``),i.innerHTML=`<span style="font-weight:700; color:${e===`관리자`?`#3b82f6`:`#64748b`};">${e}:</span> ${t}`,n.appendChild(i),r||(n.scrollTop=n.scrollHeight)};(async()=>{if(Z)try{let{data:e,error:r}=await Z.from(`live_chats`).select(`*`).eq(`live_id`,t).order(`created_at`,{ascending:!0}).limit(100);if(r)throw r;e&&Array.isArray(e)&&(e.forEach(e=>{f(e.nickname||`?`,e.content||``,!0),u=parseInt(e.created_at)||0}),setTimeout(()=>{n.scrollTop=n.scrollHeight},100))}catch(e){console.warn(`Failed to load chat history`,e)}})(),Z&&(d=Z.channel(`admin-chat-channel-${t}`).on(`postgres_changes`,{event:`INSERT`,schema:`public`,table:`live_chats`,filter:`live_id=eq.${t}`},e=>{let t=e.new;t&&parseInt(t.created_at)>u&&(f(t.nickname||`?`,t.content||``,!1),u=parseInt(t.created_at))}).subscribe()),_.addEventListener(`adminTabLeave`,()=>{d&&Z.removeChannel(d),s&&clearInterval(s)})},E=()=>c.map((e,t)=>{let n=e.clicks||0;return`
