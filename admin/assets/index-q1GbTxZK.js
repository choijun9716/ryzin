...
감사합니다.`}])},300)})}async function bt(e){let t=vt();if(!t.linkId||!t.secretKey)return console.warn(`팝빌 API 정보가 설정되지 않아 모의(Mock) 발송으로 처리됩니다.`),new Promise(t=>{setTimeout(()=>{t({success:!0,isMock:!0,receiptNum:`mock_receipt_`+Date.now(),message:`${e.receivers.length}건 발송 요청 완료 (모의)`})},1e3)});try{return console.log(`팝빌 전송 페이로드:`,e),{success:!0,receiptNum:`live_receipt_`+Date.now(),message:`발송 완료`}}catch(e){throw console.error(`팝빌 연동 오류:`,e),Error(`팝빌 메시지 전송에 실패했습니다: `+e.message)}}function xt(){let e=document.createElement(`div`),t=[],n=null,r=[],i={},a=[];function o(){let o=V.getAll(`hosts`)||[],s=V.getAll(`projects`)||[],c=new Set(s.map(e=>e.brand).filter(e=>!!e)),l=Array.from(c).map(e=>({id:`brand_`+e,name:e}));e.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
          <div>
