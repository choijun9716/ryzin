    ...
    <div class="product-row" style="${r?`border: 2px solid #2563eb; background: #f8faff; box-shadow:0 4px 12px rgba(37,99,235,0.08);`:``}">
      ...
      <div class="product-inputs">
        ...
        <div style="display:flex; align-items:center; gap:8px; margin-top:8px; background:#f8fafc; padding:8px 12px; border-radius:10px; border:1px solid #e2e8f0;">
          ...
          <input type="text" class="modern-input" style="flex:1; padding:6px 10px; font-size:12px;" value="${e.detailImage||``}" data-idx="${t}" data-field="detailImage" placeholder="이미지/GIF URL 직접 입력 또는 우측 파일 업로드 (쉼표 구분 다중 지원)">
          <input type="file" id="upload-detail-${t}" accept="image/*,.gif" style="display:none;" data-idx="${t}" class="prod-detail-upload">
          <button type="button" id="btn-upload-detail-${t}" class="action-btn" onclick="document.getElementById('upload-detail-${t}').click()" style="padding:6px 12px; font-size:12px; font-weight:700; background:#0f172a; color:#ffffff; border:none; border-radius:8px; cursor:pointer; white-space:nowrap;">
            ${e.detailImage?`+ 이미지/GIF 추가`:`+ 이미지/GIF 업로드`}
          </button>
          ${e.detailImage?`
            <a href="${String(e.detailImage).split(`,`)[0].trim()}" target="_blank" style="font-size:11.5px; color:#2563eb; text-decoration:underline; font-weight:700; white-space:nowrap;">미리보기</a>
            <button type="button" class="btn-del-detail-img" data-idx="${t}" style="background:transparent; border:none; color:#ef4444; font-size:12px; font-weight:700; cursor:pointer; padding:0 4px;">전체 삭제</button>
          `:``}
        </div>
        ${i?``:`
