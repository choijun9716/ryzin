const fs = require('fs');
const file = '/Users/chaeijun/라이브 관리/src/pages/projects.js';
if (fs.existsSync(file)) {
  let content = fs.readFileSync(file, 'utf8');

  // Step 1: Replace variables
  const oldVarsStart = `  let filters = { status: '', brand: '', platform: '', month: '', category: '', settleStatus: '' };
  let colGroups = { basic: true, host: true, result: false, finance: false };`;
  const newVarsStart = `  let filters = { status: '', brand: '', platform: '', month: '', category: '', settleStatus: '' };
  let colGroups = { basic: true, host: true, result: false, finance: false };
  let currentView = 'list'; // 'list' or 'calendar'
  let calendarDate = new Date();`;
  content = content.replace(oldVarsStart, newVarsStart);

  // Step 2: Replace container.innerHTML to handle both views and add toggle buttons
  ...
  // Step 3: Ensure the closing tag of the view condition is placed correctly
...
`;
  ...
  // Step 4: Add event listeners for new buttons
  ...
  console.log('User directory update complete.');
}
